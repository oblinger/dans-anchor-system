...
| -[[DAS Move Skill]]- | : `/move` relocates an anchor folder to a new path and updates every path-dependent system that indexes it — HookAnchor, Claude Code session history, hardcoded paths inside the anchor's own configs, …<br>→ [[DAS]] → [docs](hook://docs) → [DAS Move Skill](hook://p/DAS%20Move%20Skill)  |
| --- | --- |
