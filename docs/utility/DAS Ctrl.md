...
| -[[DAS Ctrl]]- | → [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[DAS]] → [DAS Ctrl](hook://p/DAS%20Ctrl)<br>: How to control the local environment — browser, shell sessions, and system interaction|
| --- | --- |
...
The underlying mechanism is `ctrl`, a Python script the user has wired onto their `$PATH`. It uses tmux for persistent sessions and Safari for browser automation.
