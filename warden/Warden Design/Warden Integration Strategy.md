...
Two genuine additions Vale motivates in our format: an in-file **scope/region** dimension on `where::`, and a **severity** axis. Both are clean extensions (downstream touches to [[Warden Rule]] / [[DAS Ruleset]]).
...
- **D8 — Our language expresses Vale's *capability* in plain Python, not a host for its *syntax*.** Native checks are arbitrary Python `if::` (trivially a superset of Vale's fixed check-types); Vale is an **opt-in `sh` adapter** for the NLP/spelling/script/metric niches (native for the regex/structural subset). The genuine format additions are an in-file `scope`/region dimension on `where::`, a `severity` axis, message interpolation, and `link`. Integration = a Vale **importer** (1:1 mapping) + the opt-in adapter. We do **not** accept Vale's YAML as a surface dialect. Downstream: [[Warden Rule]] / [[DAS Ruleset]] (severity + scope), [[F211 — Rule compiler and installer]] (the adapter).
- **D9 — Two Vale on-ramps: a zero-migration compat mode *and* an importer.** A user with existing Vale rules must not have to abandon them to adopt their first Warden rule. So Vale interop has two complementary paths: **(a) compat mode** — Warden auto-detects the user's `.vale.ini` and runs it through the **Vale binary** at audit time (Vale parses its own files; we collect its JSON findings and surface them) — *zero migration*, opt-in, detect-if-present, audit-path only (D3); **(b) the importer** — converts Vale rules into native Warden `if::` rules for hot-path execution and dropping the Vale dependency. The user *coexists day one, migrates whenever* — rule by rule. This does **not** relax D8: we never parse Vale's YAML ourselves — the Vale binary does — so Vale is never a surface dialect anyone authors in. Cost is low: compat mode is the D3/D8 adapter pointed at an existing config + auto-detect.
