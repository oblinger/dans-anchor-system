...
**Compiler output — dual form** *(F211 Q2/Q3, 2026-07-02).* The index is **two coordinated artifacts**: a **shared IR / data-table** for the declarative surface (moment dispatch, `where::` selectors, fixed-vocabulary `if::` guards), interpreted *identically* by the Python reference and the Rust hot-path engine (so the differential harness stays honest); and **emitted Python modules** for the inherently-Python bits (arbitrary `if::` / body). Rule Python is allowed at **every** moment — the warm daemon makes a quick check an **in-process sub-ms call**, not an interpreter launch — so the veto-path limit is a **per-moment operation-cost budget** (plus an authoring lint on slow ops), not a language ban. Rust interprets the IR in-process and **delegates a rule's Python bits to the daemon** (it never embeds CPython); the rare Python-carrying `tool:pre` rule pays a bounded IPC hop while the declarative majority stays fully in-process.
...
- **Fire time** — resolve the **anchor** from the path (the written / considered file, or the triggering agent's cwd) via a cached **reverse index** that walks up to `.anchor`. This is needed anyway to bind `{ANCHOR}` / `{slug}`, so it is not extra work. The anchor's **active-set** is the union of its traits' sets.
