...
7. **Doc-fire on write** *(next)* — wire the doc-fire (audit-on-write) path into the live `write:markdown` moment, so an anchor's doc-audit rules fire on a real markdown edit and steer the agent, not just on an explicit `/audit`. The doc-fire executor already exists and is verdict-identical to `audit-plan` ([[F212 — Python reference implementation|F212]] `warden_docfire.fire_audit`); this item is the **dispatcher wiring + trait-gating** (which anchors' writes run doc-audit) + a re-evaluation guard so a single edit doesn't re-run the whole umbrella noisily. Adopt on the Warden anchor first (lowest blast radius), widen from there. → new feature doc (mint on start).

8. **Markdown-discipline / progressive-disclosure stress-test rules** *(after 7, before Rust phase 2)* — the deliberate **hard case** that stress-tests the whole system, chosen by the user *("a great case to really push on … I will see the system's behavior in the way it lays documents out")*. This is the family of document-layout rules — **markdown discipline, progressive disclosure, document organization** — where a rule is *conditional and multi-check* (one rule performing several structural checks), e.g.:
   - **Dispatch table present/absent by context** — the dispatch table (masthead) must be present on an anchor page and **absent** on a non-anchor file; never both a dispatch table and a `:>>` breadcrumb; never a dispatch table on a non-anchor (per [[feedback_breadcrumb_vs_dispatch_table]] / `R-anchor-page`). A rule that checks *"is this the kind of doc that should have X, and does it?"* both ways.
   - **Blank-line placement** — the required blank lines are in the right spots (progressive-disclosure section spacing, definition-list shape, TOC/masthead framing).
   - **Multi-check rules** — a single rule that runs several such structural checks together, rather than one check per rule.
   Value: these are exactly where a declarative rule engine earns its keep over flat linters — conditional presence, layout-sensitivity, and multi-assertion bodies — so they are the real proof that Warden can *shape* how documents are laid out. Build them, fire them live (on write, per item 7), and watch the system keep the user's documents in canonical form. → new feature doc (mint on start).

9. **Rust engine, phase 2 (resident daemon)** *(after 8)* — the warm resident-Python interpreter + IPC so the Rust binary owns the **whole** hot path (`body_py`/`guard_py` execution, not just selection), then wire the Rust selection path into the live dispatcher. Pure perf, no behavior change; a fresh-session-scale systems build. Architecture lean: a portable socket daemon (not embedded PyO3), per the packaged-app rule. → [[F213 — Rust performance implementation + ms budget|F213]] phase 2.
