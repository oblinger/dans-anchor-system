...
1. **Kill switch first** ✅ *(done 2026-07-02)* — an instant, global, no-edit disable so a broken rule pulls out of every environment in one move. → [[F220 — Live hook install + kill switch|F220]].
2. **Live hook install (pilot surface)** ✅ *(done 2026-07-02)* — the compiled engine is wired into the real `settings.json` hook surface; live steer-only on the pilot, inert-by-default. → [[F220 — Live hook install + kill switch|F220]].
3. **Live-integration test class** ✅ *(done 2026-07-02)* — a `warden-selftest` ruleset + funky trait + a harness that drives a real agent and proves the hook fired via the log; each case run on/off to prove the kill switch. Caught the live-only path-quoting bug on its first run. → [[F221 — Live-integration test class|F221]].
4. **Rust engine, pulled forward** ✅ *(phase 1 done 2026-07-02)* — the Rust **selection engine** (`warden/rs/`, `warden-rs`) runs against the Python reference oracle, **byte-for-byte identical** across the live corpus × trait sets + synthetic guard/action fixtures (standing differential gate + `cargo test` + CI); cold bench **2.65 ms** incl. full IR parse, ~7.5× the Python cold path. Phase 2 = the resident-Python IPC (daemon) for `body_py`/`guard_py` execution, then wiring the Rust selection path into the live dispatcher. → [[F213 — Rust performance implementation + ms budget|F213]].
5. **Moment-corpus golden case** — extend the golden corpus to the moment fire path (not just doc-audit), tested live on the system. *(Moment firing is already covered at the unit (`test_warden_fire`), live (`live-e2e`), and Rust-differential layers; this adds the corpus-schema-blessed form — needs a moment-stream corpus-shape decision.)*
6. **Keep going to real use** — widen the live surface, dogfood a real useful rule on a real anchor (the doc-fire audit-on-write path — needs a doc-rule trait-gating decision), **fix breakage live** behind the always-available kill switch.
