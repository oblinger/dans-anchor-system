...
| EX | Rule | Target | Grade | Justification |
| --- | --- | --- | --- | --- |
...
| EX002 | R-one-path-03 | ** | A | Same deliberate two-implementation architecture as EX001; split into its own row so each rule can be retired independently. |
| EX003 | R-interfaces-folder-01 | ** | B | **No `interfaces/` package.** The engine is a small procedural pipeline (9 modules, few classes); its real contracts are data schemas — `rules-ir.json`, the daemon IPC protocol, the hook JSON — not abstract types. Revisit if the engine grows a second implementation of any internal component. |
| EX004 | R-interfaces-folder-02 | ** | B | Same reasoning as EX003. |
| EX005 | R-factory-pegboard-01 | ** | B | **No factory-pegboard.** The one genuine implementation swap (Python ↔ Rust dispatcher) already goes through a registration-style switch (`warden install --rust`), not call-site edits. |
| EX006 | R-factory-pegboard-02 | ** | B | Same reasoning as EX005. |
| EX007 | R-factory-pegboard-03 | ** | B | Same reasoning as EX005. |
