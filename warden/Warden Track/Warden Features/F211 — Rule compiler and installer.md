...
**Remaining (downstream / content-gated, not this slice):** (a) **live-hook install** — registering the compiled moment-modules with the real Claude Code hook surface (vs. the engine-level `fire()` the tests drive) is the productionisation that rides **M4** (migrate existing surfaces); (b) **full-anchor active-set from real traits** — the multi-file `include::` DAG flatten (reuse `audit-plan.flatten_umbrella`) needs the **trait→ruleset `include::` bindings authored in the trait specs**, which is [[F218 — Design-rules catalog — ship with skills, adopt per-application|F218]] content on a propose→review gate (not free agent authoring); (c) **tier doc-rule emission** (the deferred R-query-01..13) + the **recompile cache** (skip on unchanged active-set + rules) at the from-index compile level. The pilot ruleset was self-contained, so (a)/(b) were not on the pilot's critical path — the engine is complete; exercising it across the live vault is gated on M4 + F218.
