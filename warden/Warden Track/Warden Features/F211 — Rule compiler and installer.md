...
The engine that makes active rules fire **implicitly** at runtime moments — a *compiler*, not an interpreter. It resolves the rules active for an anchor — a pure function of its `.anchor` **traits**, each trait pulling in its omnibus rulesets — indexes each onto its dispatch key (by `when::` moment, or by `where::` place), and **pre-compiles** all rules sharing a moment into one fast module (generalizing today's `/distill`). At fire time the hook subsystem ([[DAS Audit Architecture|Audit Architecture]]) runs the compiled module for that moment, which checks the residual conjunction and emits steers/fixes. This is the hot path the millisecond budget rides on.
...
**IR schema specced 2026-07-02** (§ IR schema) — the compiler's concrete output form: the shared `rules-ir.json` table (moment dispatch index + per-trait active-sets + rule rows with declarative fixed-vocab guards/actions and `guard_py`/`body_py` escapes) plus the emitted `rules_<anchor>.py` module, worked end-to-end through `R-query-14`. This resolves [[F212 — Python reference implementation|F212]]'s open module-representation question.

**Compiler built + pilot compiles 2026-07-02** — `warden/engine/warden_compile.py` implements the parse → clause-split → emit half of the compiler, ported through the `R-query-14` pilot. It parses the **executable when-rule** grammar (F180: a `### RULE … (when:: <moment>)` heading + `when::`/`where::`/`if::` fields + a fenced ```python``` `trigger`/`guard`/`body`) — a superset of the tier-audit grammar `audit-plan.py` parses — then clause-splits each rule into its IR row (`canonical_moment` applies the F209 phase default / honours an explicit phase; `parse_if` sorts `if::` into the F210 fixed-vocab `guards` vs. a `guard_py` residual; a computed body → `body_py`) and **emits both artifacts** (`rules-ir.json` + `rules_<anchor>.py`, entry def renamed to `body_<id>`, imports hoisted). Compiling `R-query` yields **exactly** the § IR schema worked example — `R-query-14` keyed under `moments["skill:post:audit-q"]` and `traits["query"]` with `where:null`/`guards:[]`/`body_py:"body_R_query_14"`, the emitted `body_R_query_14(ctx)` reproducing today's `audit-q` autofire steer; the 13 tier doc-rules (R-query-01..13) are honestly recorded in the IR `deferred` list, not silently dropped. Pinned by `warden/engine/test_warden_compile.py` (IR-row + emitted-body + stats, all green). As part of the port, `R-query-14`'s authored `when::` was brought to the frozen phase-explicit form (`skill:audit-q` → `skill:post:audit-q`).

**Fire path built + Success Criteria core met 2026-07-02** — `warden/engine/warden_fire.py` is the moment dispatcher: `load_compiled` loads the IR + imports the emitted module; `build_ctx` assembles the moment-shaped interpretation environment (for the `audit-q` cooperative post-moment: `queries_text` + `git_aspect` sensed from `.anchor` traits + `anchor`); `fire(moment)` is a dict lookup on `moments[moment]` → active-set membership filter (`is_active`: rule's keying trait ∈ the anchor's `traits:`) → declarative `guard` eval (`eval_guard`, F210 vocab) → run the emitted `body_<id>` / declarative `action` → steers. `warden/engine/test_warden_fire.py` (green) pins **both** Success-Criteria properties: the real `R-query-14` fires end-to-end at `skill:post:audit-q` against a fixture anchor (adopted `query` trait + Commit aspect, a commit/push question in its queries file) → the exact autofire steer; and a synthesized two-rule fixture on different moments proves **indexed dispatch** (firing one moment runs only its module) + **active-set gating** (an anchor that has not adopted the rule's trait fires nothing). The compile→install→fire loop is real and tested through the pilot.

**Corpus compile + tier-rules + recompile cache built 2026-07-02** — `compile_corpus` (`--root`) consumes the `warden_scan` index and compiles **every** ruleset-bearing file into one combined IR (live repo: **449 rules / 116 rulesets / 112 files** — 1 when-rule, R-query-14, + 448 doc-rules); `emit_module` encapsulates each rule's helper constants inside its function so two rulesets never collide at corpus scale (the 449-rule module imports clean). All tiers now emit: when-rules → `moments`, tier doc-rules → `doc_rules` with declarative `check`/`judge`/`track` actions delegating to audit-plan's checker registry by name. The **recompile cache** keys on the scan-index content hash (`source_hash`) — an unchanged corpus is a cache hit and skips the compile (verified miss→hit→force-miss), closing the last literal Success Criterion. All pinned by `test_warden_compile.py` (6 tests, green). The reference loop that ties scan→compile→fire is [[F212 — Python reference implementation|F212]]'s `warden_engine.py`.

**Engine complete; only live-hook install (M4) remains.** Every Success Criterion is met at the engine level — compile (single + corpus), install-and-fire (`R-query-14` end-to-end + indexed dispatch + active-set gating), recompile cache hit. The sole remaining piece is **wiring the compiled moment-modules into the real Claude Code hook surface** (vs. the engine-level `fire()` the tests drive) — a live, high-blast-radius rewrite of the `audit-q` autofire / F091 `compact`+`markdown-write` hooks that rides **M4** (migrate existing surfaces) and deserves its own studied session. Vault-wide *active-set adoption* (which anchors adopt which rulesets) also awaits the trait→ruleset `include::` bindings authored per [[F218 — Design-rules catalog — ship with skills, adopt per-application|F218]] (propose→review), but the engine already resolves active-set from whatever traits an `.anchor` declares.
...
**Carry-outs for § Design:** (1) `root` becomes an engine config parameter — today's engine hardcodes the dans-anchor-system repo root; generalize to the vault root (all 112 current ruleset files happen to live in dans-anchor-system, but the anchor-embedded future won't). (2) Index schema: `{path, mtime_ns, size, ruleset_names[]}` + the flattened-rules content hash F214's corpus already pins.
