...
**Remaining build:** (a) **install + fire** — register each moment's compiled module with the runtime hook surface + a `ctx`-constructing dispatcher, firing `R-query-14` at the `audit-q` cooperative post-call; (b) the multi-file **`include::` DAG flatten** for a real trait umbrella (reuse `audit-plan.flatten_umbrella` — the pilot ruleset was self-contained) + **active-set resolution** from a live `.anchor`'s `traits:` + the implicit base trait; (c) **tier doc-rule emission** (the deferred R-query-01..13 — declarative `check::`→`action` / agent-judge rows). Next crank slice: (a), the fire path, which closes the F211 Success Criteria.
