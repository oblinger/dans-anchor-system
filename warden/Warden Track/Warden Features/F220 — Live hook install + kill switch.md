...
Remaining: widening the registered surface past the steer-only pilot, and wiring the doc-fire (audit-on-write) path into the dispatcher so a real, useful rule fires (the genuine dogfood — needs a trait-gating decision for doc-rules).
