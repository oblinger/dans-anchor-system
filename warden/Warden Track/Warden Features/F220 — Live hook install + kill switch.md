...
**Global go-live done 2026-07-02.** `warden install` merged the 4 pilot hooks into the user's real `~/.claude/settings.json` (all 7 pre-existing hooks — bash-guard, webfetch-authwall, maintain, audit-on-write, load-role, messages-stop — preserved; valid JSON; backed up to `settings.json.pre-warden.bak`), compiled the corpus to `~/.warden/`, and symlinked the `warden` CLI into `~/bin`. Verified end-to-end against the **global** config (no project override): a real agent in a `warden-selftest`-trait anchor fired all 4 moments live, and `warden off` on the real install → zero firings. It is **inert-by-default** — the only moment rules are `R-query-14` (`skill:post:audit-q`, which the v1 dispatcher never emits) and the selftest rules (`warden-selftest` trait, which no real anchor declares) — so it establishes the live plumbing with zero behaviour change to real sessions until a real anchor adopts a Warden trait. Overhead is ~20 ms/hook (Python startup; IR load negligible). **Cannot block** a prompt or tool: `warden_hook.py`'s top-level imports are all stdlib and `main()` returns 0 on every path (the quoted-path fix closed the one launch-failure block route).

**First real dogfood live 2026-07-02.** Past the selftest-only pilot: `R-warden-dev` (`warden/Warden Track/Warden Dev Ruleset.md`) is the first **genuinely-useful** rule firing live — a `session:start` steer that orients any agent working in the Warden anchor to its dev disciplines (Commit mode, run the unit + corpus suites before committing engine changes, keep the Rust↔Python differential green, `warden off` to disable). Keyed by the `warden-dev` trait, which `warden/.anchor` adopts; proven through the **real dispatcher** (`warden_hook.py`): a SessionStart event with `cwd` in the Warden tree emits the steer as `additionalContext`, it is inert in a non-`warden-dev` anchor, and `warden off` no-ops it. Warden now audits its own dev workflow with its own engine. The moment-corpus golden re-pinned to the new surface.

Remaining: wiring the **doc-fire (audit-on-write)** path into the `write:markdown` dispatcher so doc-audit rules fire live on edits (a broader, noisier surface — needs a trait-gating + when-to-fire decision), and widening adoption to more anchors.
