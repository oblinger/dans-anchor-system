...
**All five layers live 2026-07-05 — the regime is complete.** With [[F213 — Rust performance implementation + ms budget|F213]] phase 2 landed, the last layer (performance) joined CI: `test_warden_perf.py` benches the live dispatcher end-to-end (`warden-rs hook`, fork+exec, mean of 30) in both shapes — no-fire (3.22 ms local) and Python-body-firing through the resident daemon (3.49 ms) — and **fails on hot-path regression** (CI-tolerant ceilings 20/30 ms sit an order of magnitude under a Python-startup regression; the PRD ms budgets are held and recorded in the F213 local bench). The differential layer also gained its live-dispatcher gate: `test_warden_hook_rust.py` (Rust hook ≡ Python hook, 7/7 cases). Standing CI: 8 unit files + golden corpus (both engines) + two differential gates + the perf gate; the live-e2e layer (F221) remains the local/manual gate.

## Resolved

- **Q1 — CI home** — **Resolution (user, 2026-07-01): (B) — stand up GitHub Actions in dans-anchor-system now.** Rationale: good practice getting the rig built; migrating a working workflow to the extracted Warden repo later is trivial. Overrides the filed Lean (A). Realized as `.github/workflows/warden-tests.yml` — golden-corpus job on push/PR touching `warden/**` or the audit engine, plus manual dispatch; the runner's exit code is the gate. Future layers (unit at the build milestone, differential+perf at the Rust milestone) join the same workflow.
