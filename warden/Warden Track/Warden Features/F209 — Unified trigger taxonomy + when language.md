...
**Drafted 2026-06-26.** Taxonomy spec authored at [[Warden Events]]; architecture §5 updated to defer to it. **All questions resolved 2026-07-02** (user): Q1 phase default (`tool`→`post`, `skill`→`pre`), Q2 `git:*` first-class, Q3 `skill:pre` now / `skill:post` V2/V3.

**Frozen 2026-07-05 — the M0 `when::` surface is locked and verified against the live dispatcher.** Every legacy trigger surface maps to a canonical, dispatcher-reachable moment: `compact` → `session:compact` (PreCompact), `markdown-write` → `write:markdown` (PostToolUse Write/Edit), `skill:*` → `skill:pre:<name>` (PreToolUse Skill). The no-orphans check found one real orphan — `R-query-14` keyed verbatim at `skill:post:audit-q`, a moment the dispatcher never fires — fixed by realizing the Q3 resolution in the compiler: `canonical_moment` now normalizes an authored `skill:post:X` to `skill:pre:X` (v1 accepts post, treats it as pre; the post ladder stays V2/V3). The property is pinned by a standing test (`test_warden_compile` — every compiled moment key must be reachable from `warden_hook.event_to_moments`), so a future rule authored onto an unreachable moment fails CI instead of silently never firing. Live corpus recompiled; `skill:pre:audit-q` verified in the installed IR.
