...
- **M2 — Python reference implementation** [Ready] — **reference loop built 2026-07-02** (`warden_engine.py` — lazy scan→compile→fire, the behavioral oracle; `R-query-14` fires through it). Remaining: the stable verdict/steer **snapshot API** the differential harness records + mapping the in-process dispatcher to real hook events for e2e. Reuses `audit-plan.py`. → [[F212 — Python reference implementation]]
