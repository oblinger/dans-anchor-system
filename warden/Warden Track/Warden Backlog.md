...
- **F209 — Unified trigger taxonomy + `when::` language (M0 freeze)** [2 Questions] — → [[F209 — Unified trigger taxonomy + when language]] · the skill-emission question resolved 2026-07-02 (user): `skill:pre` ships in v1 (from the Skill-tool invocation), `skill:post` deferred to a later version with an approximation ladder ([[Warden Roadmap]] § Beyond v1); v1 collapses `skill:post`→`skill:pre`. Two questions remain: phase default for bare moments (tool side), `git:*` first-class vs derived. Then map every existing trigger surface (`compact`, `markdown-write`, `skill:*`) onto a canonical moment path with no orphans.
