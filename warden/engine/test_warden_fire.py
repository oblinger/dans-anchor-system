...
Pins the two properties the F211 Success Criteria name:
  1. The real `R-query-14` fires end-to-end at `skill:post:audit-q` against a
...
def test_real_r_query_14_fires(tmp: Path):
    ...
    ctx = wf.build_ctx(anchor, "skill:post:audit-q")

    assert ctx.git_aspect == "commit", ctx.git_aspect
    assert "commit and push" in ctx.queries_text
    steers = wf.fire(ir, module, "skill:post:audit-q", ctx, traits)
...
def test_indexed_dispatch_and_gating(tmp: Path):
    ...
    assert set(ir["moments"]) == {"skill:post:audit-q", "tool:post:Write"}, ir["moments"]

    adopted = ["fix", "_base"]
    ctx = wf.build_ctx(tmp, "skill:post:audit-q")  # tmp has no .anchor → bare ctx

    # firing audit-q runs ONLY R-fix-01
    a = wf.fire(ir, module, "skill:post:audit-q", ctx, adopted)
    assert a == ["A-fired"], a
    ...
    none = wf.fire(ir, module, "skill:post:audit-q", ctx, ["other", "_base"])
    assert none == [], none
