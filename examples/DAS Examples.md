...
**Nine facets have no example at all** — `Anchor`, `Dispatch`, `Dispatch Table Design`, `Design Docs`, `Code`, `Doc`, `Output`, plus the proposed `Primitives` and `Aspects`. Seven of the nine are the *broadest* facet in their subsystem, which is a recognisable failure shape: the general spec is the hardest one to exemplify, so it gets skipped while its specialisations get covered.
