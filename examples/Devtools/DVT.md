...
| -[[DVT]]- | : Devtools — example grouped-dispatch collection (> 15 members)<br>→ [[DAS]] → [[FEX]] → [DVT](hook://p/DVT)  |
| --- | --- |
| Related | [[BRDG\|Bridges]] (flat variant),  [[DAS Dispatch Table]],  [[DAS progressive-disclosure]],  [[FEX]], |
...
> - **Each group-row label is a link** (`[[DVTB|Build]]`) *down* to that group's own page. The members beside it are a hand-pinned **preview**; the full list lives on the group's page.
> - **`+` marks the label as an expandable container**, not a leaf. That marker is what separates this shape from a [[HRUN|grouped spine]], where the labels are plain text and every child is already in this folder.
