---
description: "testing strategy + proposed-tests overview"
status:: drafting
---
# CAE Testing

| -[[CAE Testing]]- | : <br>→ [[KM]] → [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[CAE]] → [[CAE Design]] → [CAE Testing](hook://p/CAE%20Testing) |
| --- | --- |
| [[CAE PRD]] | product requirements that drive the e2e inventory |
| [[CAE Architecture]] | subsystem boundaries that drive integration inventory |
| [[CAE Decisions]] | injectable Clock + central retry (D01, D03, D5) make deterministic testing possible |
| [[CAB Testing]] | facet spec this doc follows |
| --- | |

**TL;DR.**

- **Posture** — heavy unit + integration, modest e2e, property-based for two load-bearing invariants.
- **Determinism** — single-process scheduler; `Clock` is injectable, no test reads wall time.
- **Four kinds** — unit / integration / e2e / property-based. No smoke, perf, or fuzz in v1.
...
- **Property bar** — two invariants: scheduler picks highest-priority Ready, retry delay sequence non-decreasing to cap.
- **Tier mapping (per [[verification]])** — Tier 1 = CI (unit + integration + property); Tier 2 = e2e on push + property nightly; Tier 3 = author dogfooding.
...
- **Unit** — every public function in `src/execution/`, `src/retry/`, `src/store/`, `src/clock/` has at least one unit test exercising its golden path. Edge cases are added as bugs surface; not pre-targeted.
- **Integration** — every subsystem boundary in [[CAE Architecture]] has at least one integration test:
  - Scheduler ↔ TaskStore
  - Scheduler ↔ RetryManager
  - Scheduler ↔ Worker Pool
  - RetryManager ↔ TaskStore (dead-letter persistence)
- **End-to-end** — exactly one e2e per user story in [[CAE PRD]] § User Stories. Three e2e tests total at v1; grows as new user stories are added.
- **Property-based** — two properties, both load-bearing for correctness claims:
...
| ------------------------------------- | ----------------------------------------------------- | --------------------- |
| `e2e_schedule_a_task`                 | US-1: `cae schedule "<cmd>" --at <time>` lands a task | [CAE E2E § US-1]      |
| `e2e_monitor_task_status`             | US-2: `cae status` groups Pending/Running/Done        | [CAE E2E § US-2]      |
| `e2e_failed_task_retries_then_visible` | US-3: failing task retries, shows in `cae history`   | [CAE E2E § US-3]      |
...
- [[CAB Testing]] — facet spec this doc is the worked example for
...
- [[verification]] — four-tier verification discipline mapped above
