...
| -[[Harbor Releases]]- | : canonical stream-spine exemplar — dated children, newest first<br>→ [[DAS]] → [FEX](hook://FEX) → [Harbor Releases](hook://p/Harbor%20Releases)  |
| --- | --- |
...
> - **Single-file form** — entries are H2s inside one document. There are no children to enumerate, so the page ends `...`, not `^^^`. Both [[DAS Log]] and the `{slug} Log` template are in this form.
> - **Folder form** — one dated file per entry, which is what this page demonstrates. *That* is where the stream spine appears.
>
> So the caret tracks **dated children across files** while the discipline also governs **dated entries within one file**: same ordering, different object. The corpus bears this out — of the vault's stream-named pages only 4 end `^^^`, while the 23 pages that do end `^^^` are mostly not logs at all (`Apply`, `Find`, `Guide`, `Profile`, `Survey`, `Trips`). **The marker follows the children, not the topic.**
