---
description: "example list-dispatch collection (≤ 15 members)"
---

| -[[Bridges]]- | : example list-dispatch collection (≤ 15 members)<br>→ [[DAS]] → [[examples]] → [Bridges](hook://p/Bridges)  |
| --- | --- |
| Related | [[Devtools]] (grouped variant),  [[DAS Dispatch Table]],  [[DAS Examples]], |
| [[Bridges Studio\|Studio]]  | the M2 Studio — Full Disk Access, the build box |
| [[Bridges Air\|Air]]  | the travel laptop — Syncthing peer |
| [[Bridges Beast\|Beast]]  | the GPU box — remote experiments via `exp` |
| [[Bridges Pi\|Pi]]  | the always-on Raspberry Pi — cron + watchers |
| ... | [[_{{DISK_LABEL}} Template]],  [[_{{PURCHASE_DATE}} {{HOSTNAME}} Template]],  [[Clarifier]],  [[CSE]],  [[DAS US-CAE-1 — Schedule a Task]],  [[DAS US-CAE-2 — Monitor Task Status]],  [[DAS US-CAE-3 — Retry Failed Tasks]],  [[ESP]],  [[Espresso]],  [[FEX Agenda]],  [[FEX API]],  [[FEX API Design]],  [[FEX Architecture]],  [[FEX Claude]],  [[FEX Completed Roadmap]],  [[FEX CSE]],  [[FEX Decisions]],  [[FEX Decisions Details]],  [[FEX Dispatch Examples]],  [[FEX Facet]],  [[FEX Figure Page]],  [[FEX Files]],  [[FEX Icebox]],  [[FEX Inbox]],  [[FEX Minimal Facet]],  [[FEX Minimal Skill]],  [[FEX Project Root]],  [[FEX Repo]],  [[FEX Roadmap]],  [[FEX Rules]],  [[FEX Scheduler]],  [[FEX Skill]],  [[FEX Spine Examples]],  [[FEX Stories]],  [[FEX System Design]],  [[Forum Stories]],  [[Harbor Hops]],  [[Harbor Latency Budget]],  [[Harbor Releases]],  [[Harbor Runbooks]],  [[HBR]],  [[HBR PRD User Stories]],  [[HWP]],  [[Knots]],  [[Mini]],  [[Snap]],  [[Viz Bench]],   |

# Bridges
The machines this host can reach — a small, flat collection.

> [!note] Canonical curated spine
> A hub whose children are **listed by hand, one row each**, with the `...` kept as a safety valve rather than as the content.
> - **The catchall should stay nearly empty.** Every machine here has an authored row; the `...` exists so that a fifth machine added tomorrow still shows up instead of vanishing silently.
> - **It is not a [[Harbor Hops|list spine]].** These rows are hand-written *above* the marker; a list spine's rows are machine-written *below* a `---`. Same visual result, opposite ownership — and only the machine-written kind stays correct on its own.
> - **It is not [[Harbor Runbooks|grouped]] either**, because four machines do not fall into cohesive groups. Grouped is the special case of curated where real groupings exist; inventing them here would add structure the material does not have.
>
> The live counterpart is [[Legal]] — about a hundred hand-listed contracts and invoices that genuinely do not sort into tidy piles.
