...
## Ready
- [[HBR004 - Cache eviction|T004]] — **Next:** Add `cache_max_bytes` to `harbor.toml`, then sweep least-recently-served segments on each segment write once the dir exceeds it.
...
- [[HBR005 - Watched-root hot reload|T005]] — **Next:** Watch the configured roots with an fs-event watcher and re-run the Scanner on the changed subtree only, without restarting the daemon.
