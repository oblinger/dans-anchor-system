...
1. **H1** — `<slug> <dash> <Name>` (per [[DAS Anchor Page]]): identity + jump-key. *(This example's name has no short slug, so its H1 is just the name; a slugged anchor would read e.g. `# SKL - Skills`.)*
