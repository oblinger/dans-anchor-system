...
**Check pattern:** the `Facets`-row link set equals the sub-dispatch facet files under `FCT Dispatch/`; [[DAS Dispatch Table]] is present as the base; each member breadcrumb passes through `~~[[DAS Dispatch]]~~`.
