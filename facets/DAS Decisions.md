...
**Limits worth knowing before writing any code rule** — each measured against the engine, not inferred (Atticus 2026-08-11; HA + Tink 2026-09-01, draining the R-ha registration):

- **A `where::` file glob cannot reach a `.rs` file in the audit sweep** — `audit-plan.py`'s `enumerate_scope` is `.md`-only in anchor mode (`target.rglob("*.md")`) and is rooted at the anchor folder rather than the `code:` tree, so `R-ob-cmd-proc`'s `` `file:{anchor}/**/*.rs` `` selects nothing anywhere and never appears in a plan (measured: 94 rules over `prj/Hook Anchor`, 0 with a `.rs` target). Code rules bind to a **`when:: write:<kind>`** moment instead.
- **A ruleset-level `where::` never scopes moment rules.** It is the doc-selector: `compile_rule` applies it to tier doc-rules only, and a `when::`-bound rule takes solely its own per-rule `where::`. R-ha compiled all three rules `where: null` under a scoped header glob — and so does `R-fct-claude`, the corpus's own reference write-moment set.
- **A per-rule `where::` on a moment rule IS enforced at fire time** (since 2026-09-01, warden `82b9949`, closing [[Tink Backlog#^T633|T633]]): both engines fnmatch it against the event's file path — `*` crosses slashes, so `**/App/**/*.rs` works as read — and a fileless moment (`tool:pre:Bash`) passes the filter untouched, so scoping a fileless rule still takes an `if::` guard (`R-fct-claude-01`'s `if:: file.path != …` is the reference shape). `warden compile` now warns when a scoped header leaves an unguarded moment rule unscoped.
- **A moment rule with only prose emits nothing.** Delivery requires a python `def body`/`def trigger` or a declarative `tell::`/`deny::` action; a "Check pattern" paragraph produces zero steers, and `check::` primitives are wired for doc-rules only. A prose-only moment rule is documentation that reads as enforcement.
- **`write:<kind>` moments fire only on Write/Edit tool events** — the hook derives the kind from the written file's extension (`content_kind`), and a Bash heredoc/`cp` write raises no `write:` moment at all. Bash-written files are reached only by the doc-tier governed-paths sweep (F297 leg 2). A code rule meant to catch agents working in bypass mode (where Bash writes dominate) is not covered by `write:<kind>` alone.
