...
- **A per-rule `where::` on a moment rule compiles but is not enforced.** It lands in the IR, yet no runtime caller applies it as a place filter (warden_fire's docstring defers to "the caller"; the rust hook, daemon, and `engine.fire` all decline). Tracked as [[Tink Backlog#^T634|T634]]. Until that lands, the **only live scoping for a moment rule is an `if::` guard** — `R-fct-claude-01`'s `if:: file.path != …` is the reference shape.
