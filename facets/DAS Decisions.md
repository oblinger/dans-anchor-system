...
>
> **Verified, not inferred** ([[Atticus|Atticus]] 2026-08-11, re-confirmed here): [[HA Rules]] declares `rules: HA Track/HA Rules.md` and authors `# RULESET R-ha`; a fresh `warden compile` reports *617 rules from 122 rulesets / 121 files*, `~/.warden/rules-ir.json` contains **zero** `R-ha-*` rules, and `hooks-ir.json` installs no `write:rust` hook.
>
> Until that is resolved, an anchor outside the corpus has no working home for local rules — which is the whole reason the convention was written. Which way it goes is [[Tink Backlog#^T362|T362]] Q1: relocate such rulesets into the corpus, teach Warden to read each anchor's `rules:` pointer, or narrow this convention to corpus-resident docs. **Do not author a companion `# RULESET` outside the corpus expecting it to run.**
