...
**A rule-level `where::` on a moment-bound rule is a path filter, and it takes the same comma list as the header.** `where:: **/HookAnchorApp/**, !**/HookAnchorApp/target/**` — a path passes when any positive pattern matches and no `!`-negated one does; fnmatch semantics, so `*` crosses slashes and `**` behaves as `*`. Two traps, both measured on R-ha 2026-09-01: `**/Foo/**/*.rs` cannot match a file directly inside `Foo/` (the `**/` between folder and filename needs a segment — `build.rs` was silently excluded; write `**/Foo/**`), and a rule whose selector rejects the path is logged by `warden log` as `considered, silent` at **`0.0 ms`** — a body that actually ran shows real time, so `0.0 ms` means the selector, not the rule, said no. Before 2026-09-02 the per-rule field took a single pattern and a comma list matched nothing without a warning.
