...
Formalized in the embedded [[R-versions]] below: artifacts use the `<version> <app>.dmg` form; the folder is flat and shared; promoted artifacts are immutable; each names a pushed tag; the store lives in the repo / an offload location, **not** the snapshotted vault.
