...
The Interface (see [[DAS Interface]]) is the **required top-level human-authored layer contract** for the anchor — every code anchor has one. Files row 1 links to it so a reader entering the file tree starts at the layer contract, not the deepest implementation file.
