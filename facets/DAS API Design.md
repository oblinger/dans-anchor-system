...
:>> [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[DAS]] → [[DAS Design Docs]] → [FCT API Design](hook://p/DAS%20API%20Design)
# FCT API Design
...
The API Design doc is the **current spec** — the contract surface today. Rationale lives in [[DAS Decisions]] or in the document's own `D-API<n>` design-decision rows (see [[#RULESET R-api|R-api]]). If the anchor has no programmatic consumer (no library form, no sub-skill invocation surface, no integration story), this facet is N/A; mark it `none` in [[DAS Status]] and omit the file.
