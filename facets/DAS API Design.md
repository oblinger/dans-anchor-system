...
`{slug} Design/{slug} API Design.md` — single-file form. Upgrade to anchor-folder form `{slug} API Design/` only when distinct sub-surfaces (e.g., library API + service API + sub-skill API in the same anchor) warrant separate files.
