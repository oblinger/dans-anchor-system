...
*Stone display is what a line shows as a **guest** on another anchor's page; on the owner's own pages the alias is a bare `-` (§ Headers, 2026-08-30). The cells above are parsed by `stone-kinds.py` and stay literal.*
...
**A stone's alias is a dash on its owner's own pages, its owner's name everywhere else** (Dan, 2026-08-30: *"If I'm in the Atticus list, I shouldn't see Atticus colon. I should just see a dash"*). In `Atticus Pebbles.md` a line reads `[[Atticus P0007|-]] vault LFS outgrown…`; the same stone in `HUD 1` reads `[[Atticus P0007|Atticus:]] …`. The owner is the page, so naming it per line is noise; a guest line has to say whose it is. The dash resolves to the page's anchor when read — for numbered stones the link target already carries the owner, and for dated ones the engine reads the dash as *this anchor* (a dashed dated line read with no page context is nobody's, never a guess). `stone update` compares the whole rendered line, so an alias that has gone stale — an old slug, or the full name where the dash now belongs — is brought current on the next pass; anything typed before the link (a list `- `, an indent) is kept. The dash is Dan's first cut (*"for now, let's just do a single dash and see what it looks like"*); a `--` form was floated for later.

**The first line of a control file's body is always blank** (Dan, same day). It is the user's typing slot: whatever is put there becomes the first line, and the next engine write opens a fresh blank above it, so the slot is never used up. A new stone lands *below* the slot. Every engine write path keeps this invariant; a list that has never held a stone gets no file just to carry a blank.
...
**Ruled by Dan, 2026-09-03 (T655), against Obsidian's ~40K-file ceiling.** A retired stone is not a file. `archive/YYYY-MM <Slug[.list]> archive.md` inside the group folder — the list named exactly as addressed — takes every stone retired that month as a `# <Slug> <id>` section — an H1, so a body's own headings never blur the boundary (Dan, same day), appended in retirement order, oldest first, no title line of its own: `line::`, its other keys, `retired:: <date>` (or `moved:: <date> → [[new name]]` for a `move --dest`), then the body verbatim; `appears::`/`enrolled::` are dropped, being live-list state. The file's `aliases:` gains each name, so `[[Tink P0020]]` keeps resolving. `archive/` is written and never read back — a month file is not a stone, and `update` loads nothing from it. Not a zip, in his words: *"having a single markdown file instead of a text file, because we can't look at a text file at all"* — grep, git and Obsidian all need to open it. Year roll-up (12 → 1) is a later verb.

**Archiving is one-way — there is no `unarchive` verb (Dan, 2026-09-03, T658).** In the rare case a stone must come back, do it by hand: copy its section out of the month file into `<Slug> <id>.md` in the group folder (keys as `key:: value` lines, body verbatim, drop `retired::`), delete the section and its `aliases:` entry from the month file, and re-add the stone's line to the control file; `stone update` then re-enrols it. The stone's last pre-archive save is also at `~/ob/data/md_history/<day>/` for 30 days. A verb was declined because the one real un-archive so far was a test.
