...
**The `where::` blocker is gone, measured 2026-08-11.** This paragraph used to say a folder-shaped facet's `where::` *"currently selects nothing"*. It selects correctly now: a `--batch` sweep over `Topic/MED` visits `MED Rocks` **as its own anchor** and emits verdicts. The earlier zero came from scoping on the *owning* anchor, which by design cannot see a facet sub-anchor ([[DAS Facet]] § folder facets) — and `--batch` is how the corpus is actually swept. Note the asymmetry the four kinds expose: a **rock** group carries its own `.anchor` and is reached as a sub-anchor, while a **pebble** group carries none and is reached inside its owning anchor's scope. The checkers resolve the owning slug the same way from either.
