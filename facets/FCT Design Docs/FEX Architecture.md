...
| [[FCT API Design]] | facet spec for {slug} API Design.md — the programmatic (code-to-code or sub-skill) user surface |
...
| [[FCT Design]] | design facet — the {slug} Design/ folder marks an anchor as following the designed-lifecycle convention; folder presence IS the signal (no trait field required) |
| [[FCT Files Architecture]] | the top-down design of a system's module & content structure — the file-tree architecture doc kind |
| [[FCT PRD]] | facet spec for {slug} PRD.md — the anchor's product requirements document |
...
| [[FCT UX Design]] | facet spec for `{slug} UX Design.md` — the human user-facing surface (CLI commands, screens, organization, naming, output shapes, error voice) |
