...
1. **H1** — `<slug> - <Name>` (per [[FCT Anchor Page]] / [[SKA Decisions|D06]]): identity + jump-key. *(This example's name has no short slug, so its H1 is just the name; a slugged anchor would read e.g. `# SKL - Skills`.)*
