...
Design holds UX Design (user-interaction shape), Interface (public-API / layer contract), Decisions, Data Model, Principles, PRD, Features, Roadmap, and design-trade-off discussion. The **system-architecture story is a Design child** (`{slug} Design/{slug} Architecture`, per [[FCT Architecture]]) — listed on the Design dispatch like the other design docs. Interface lives here (not in `{slug} User Docs/`) because its content describes a system contract, not an end-user task.
...
| Document | Part | Notes |
|----------|------|-------|
| `{slug} Architecture` (`.md` → `{slug} Architecture/` folder-doc on growth) | [[FCT Architecture]] | **A Design child** — the system-architecture story; governed by [[FCT Architecture]]. (F094's root placement reversed 2026-06-27.) |
| `{slug} UX Design.md` (or `{slug} UX Design/` if it grows) | [[FCT UX Design]] | User-interaction shape. |
| `{slug} Interface.md` | [[FCT Interface]] | Top-level layer contract — REQUIRED for Code anchors. Lives in Design (not `{slug} User Docs/`). |
| `{slug} Decisions.md` | [[FCT Decisions]] | Load-bearing rulings / invariants. |
...
| `{slug} PRD.md` | [[FCT PRD]] | Product requirements (when applicable). |
| `{slug} Features/` | [[FCT Features]] | Dated feature specs (feature docs are design artifacts). |
| `{slug} Roadmap.md` | [[FCT Roadmap]] | Implementation milestones (sequencing-design). |
