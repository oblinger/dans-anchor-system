...
| **Standing** | **links only** — to the standing documents about this person that sit beside the page, each named for **what it is about** (`Eli Psychology`, not `Eli Discussion`). See § Where a discussion goes below | only when there is one |
...
## Where a discussion goes — route by subject, then a standing sibling (2026-08-31)

A person page has a card (facts) and a `# LOG` (chronology), and Dan hit the gap himself on 2026-08-29 handing over a long call with his brother: *"I don't know that it should be in a log. It's gonna be so big… I'm not sure that we have a proper place to put, um, discussions. So, for the moment, you can just put it in in his log in his entry."* He named it a stopgap as he chose it. [[@Dave Oblinger]]'s log now carries one page-long entry that dwarfs every other, which defeats the one thing a log is for — scanning to answer *what did we last talk about*.

**Ask what the content is about before asking what shape it needs.** The Dave entry holds three different things, and only the first is his: *he has a job at American Airlines* is a fact about Dave (**Trajectory** row); *the parents are selling the ranch* is about the parents, and [[Parents Retirement]] already exists as its hub; *Dan thinks the stated reason is a spending decision* is his position on that same situation, and belongs with it. **None of the three is a discussion about Dave.** Content arriving through a call gets keyed to *who told Dan* when it should be keyed to *what it is about* — most of what looks like a missing surface is that misrouting, and a discussion surface that does not say this first will collect the same misfiled material more neatly.

**When it genuinely is about the person, it goes in a standing sibling — never a `## Discussion` H2 on the page.** The precedent is [[Eli]]: years of dated log underneath, a standing document (`Eli Psychology`) rewritten as understanding changes, on top ([[Winnie Mandate]] § Log, then synthesize). At `@entry` scale that is a sibling file next to the `@` page — which is already the register's convention, not a new one: `Dave Oblinger Gifts` sits beside [[@Dave Oblinger]], and 6 `… Gifts`, `Anthropic Core Views on AI Safety`, `Apple Products` and `Google People` follow the same `{Base Name} {Facet}.md` shape. **Name it for its subject, as Eli's is** — `Eli Psychology`, never a generic `… Discussion`; a doc named for its shape invites anything, and that is how the log filled up. The card's **Standing** row carries the link, so the page stays the index.

**Why not the third option.** A `## Discussion` H2 on the page itself is cheapest and was rejected: it puts durable understanding *underneath* a chronology that grows forever, so it sinks exactly as the log entry did. The failure being fixed is not "there is nowhere to put it", it is "it is buried" — and that option reproduces it.

Decided by [[Atticus]] 2026-08-31 on [[Winnie]]'s handoff, under [[Winnie Mandate]] § Seams. Additive: the **Standing** row is `only when there is one`, so no existing page falls out of conformance.

**Where a non-person lives — `Corp/` is every entity that is not a person** (Dan, 2026-08-29). Companies, teams (`@CV`), schools, agencies, troops all file under `AT/Corp/`; the `@` sigil already says *entity*, and the folder's one job is to separate people from non-people. There is deliberately **no `Org/`** — `ORG` is taken by the organizational-lists anchor at `_/ORG`, and a second non-person folder would buy a routing question at every mint for a handful of pages. If the distinction ever matters it is a register (`TEAMS`), not a folder. Org and group pages are folder pages and keep a dispatch table with a `...`; their opening is **not yet settled** the way the person page is — that session is still to come.

**What nesting under `Corp/@Org/` means — cohort, never employer** (Dan, 2026-08-31, resolving [[Atticus Backlog#^T286|ATT T286]] Q1). A person page filed inside a `Corp/@Org/` folder records *how Dan knows them* — the company or group they shared — not where they work now. Provenance never expires, so the path carries no as-of and needs none, and the H1's employer is *supposed* to disagree once the person moves on (all seven `Corp/@IBM/` people now show other current employers, and `Corp/@Hackers/` was never an employer at all). **No rule, checker, or staleness detector may read the path as a current-employer assertion** — measured 2026-08-31, doing so would have produced 34 false findings on day one. Orgs, by contrast, never nest: a subsidiary or acquired company gets its own flat `Corp/@X.md`, and the relationship lives on the page, not in the path.

**A superseded or absorbed org is written `[[@Old]] → [[@New]] (acquired YYYY-MM)`** — the arrow carries the succession, the parenthetical the as-of, and both sit on the card (or as the old page's forward line, pointing at the successor). Never silently replace the old name, and never leave a link resting on a dead name with no forward. Specimens: [[@Will Hsia]]'s card (`[[@Fractional.ai]] → [[@Ode]] (acquired 2026-05)`) and [[@Nick Allen]]; the old-side forward is [[@Fractional.ai]] itself.
