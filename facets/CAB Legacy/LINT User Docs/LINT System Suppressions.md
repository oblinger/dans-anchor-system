...
- **This file is NOT for per-project exceptions** — those are graded with For/Against justification under each project's own rules/exceptions surface (see [[DAS Ruleset]]); only patterns that are universally noise-not-signal belong here.
...
- **Don't pile generic markdown / vault-wide policy here** — that lives in [[R-markdown]], [[CLAUDE.md]], or the relevant [[DAS Ruleset]] facet; this page is strictly the LINT suppression matrix.
