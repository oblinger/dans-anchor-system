...
| -[[DAS Code Repository]]- | → [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[DAS]] → [[facets\|FCT]] → [DAS Code Repository](hook://p/DAS%20Code%20Repository)  |
| --- | --- |
...
A site may hold a stronger convention about how that code tree is laid
out — grouping, naming, when a folder is a container rather than a repo.
That is deliberately **not** specified here, because it varies per user
and this facet is published. If one exists it layers from the
`conventions_doc` named in `~/.config/anchor-system/global.yaml`, in a
section headed with this facet's exact name. A layered convention is
readability only: it never resolves a path, and `code:` still decides.
