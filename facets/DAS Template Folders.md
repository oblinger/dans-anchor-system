...
- **Cloned as a unit.** Copy the whole `_{Name} Template/` folder, fill/drop placeholders, add skeleton docs as they're produced. **Where it lands is declared, not derived**: the marker's `path::` names the folder and the file inside it together — `path:: {{DISK_LABEL}}/{{DISK_LABEL}}.md` — which is a destination no single filename segment could express (F570, 2026-08-20). A member whose declared path holds a **free** variable is a repeatable slot, one instance per binding, which is [[STEN Language]]'s many-by-variable default rather than a folder-template rule (R-template-10).
