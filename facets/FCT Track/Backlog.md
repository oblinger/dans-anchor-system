...
**Names are the stable key; positions are a drifting hint.** Roadmap entries and the `R` tasks that reference them resolve on the entry's **name-path**, never on an ordering number. Inserting or reordering entries drifts positions but never names, so a backlog `R` reference (and any done-log citing it) stays valid across every reshuffle. Only **renaming** an entry forces a reference sweep — far rarer than reorder/insert, which is exactly why the name (not a number) carries identity. (This is the same rationale behind [[FCT Roadmap]] R-roadmap-09's named-milestone rule.)
