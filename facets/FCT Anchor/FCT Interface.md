...
> ⚠️ **Recovery note (2026-06-14):** this `## Document Structure` table was truncated during recovery. A runaway edit ballooned the file to 19 MB (~38 K lines of one corrupted table cell full of escaped `\|` separators) and was hanging Obsidian's indexer on every launch. Only the first two clean rows survived; the remaining canonical-section rows were lost (the file was untracked — no git or backup copy existed). The full corrupted original is preserved beside this file as `.FCT Interface.CORRUPT-19MB-20260614.md` (dot-prefixed, so Obsidian ignores it). Rebuild the row list from a sibling facet such as [[FCT All Files]] when convenient.
...
- **Cross-spec consistency** — keep aligned with [[FCT All Files]] (row 1 link contract), [[FCT Module Doc]] (auto-generated vs. human-authored split), [[FCT Architecture]] (*how* vs. *what callers see*), [[FCT User Dispatch]] (location/listing), and [[SKA workflow]] (validation gate); a drift here propagates to all of them.
