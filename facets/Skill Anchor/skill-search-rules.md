---
description: skill-search-rules — facet specifying how search-family skills layer methodology, default rules, and user rules. Defaults at `<skill>`/rules/; user overrides at SRCH rules/.
applies-when-trait: Skill Anchor
location: ~/.claude/skills/`<skill-name>`/rules/  (defaults)  +  ~/ob/kmr/Topic/Search/SRCH rules/  (user overrides)
---

# CAB skill-search-rules

The skill-search-rules facet specifies *how a search-family skill (find / describe / survey / …) layers its rules — defaults ship at `<skill>/rules/`; user overrides live at `~/ob/kmr/Topic/Search/SRCH rules/`; six load layers compose at invocation with most-specific winning.* Methodology stays in `SKILL.md`; rule files are overlays on that methodology.

What distinguishes a conformant search-rule surface from one-off rule files:

- **Two homes** — defaults ship with the skill (portable); user rules stay personal (live in SRCH rules/). Skills are sharable; user preferences aren't.
...
- **User overlays optional** — all `SRCH rules/*` files are optional; absence means default behavior.
...
| **[[SRCH rules]]** (`Topic/Search/SRCH rules/`) | The user's personal overrides — same file shapes, user-specific preferences | The individual user; their custom behavior |
...
~/ob/kmr/Topic/Search/SRCH rules/           ← user overrides
...
4. SRCH rules/<verb>.md                     USER verb rules     (overrides 2)
5. SRCH rules/<noun>.md                     USER noun rules     (cross-verb override)
6. SRCH rules/<verb>-<noun>.md              USER pair rules     (most specific, wins all)
```
...
- User rule files (`SRCH rules/...`) are all optional; their absence simply means default behavior applies.
...
- A new noun (e.g. `book`) is added by creating `<skill>/rules/<verb>-book.md` in each verb skill that should support it; users can then add `SRCH rules/book.md` and `SRCH rules/<verb>-book.md` as preferences emerge.
- Pair files appear in `SRCH rules/` gradually as the user's per-pair preferences crystallize.
