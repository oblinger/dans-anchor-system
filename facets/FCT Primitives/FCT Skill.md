...
- **Keep content that belongs elsewhere out** — anchor-page format → [[FCT Anchor Page]] (the "F060 — SKILL.md is exempt" callout is the canonical pointer to where anchor-page rules diverge from SKILL.md rules); markdown-rendering → [[R-markdown]]; per-trait rules (every Skill anchor has X) → the relevant trait spec; project-wide rules → CLAUDE.md. Don't split the discipline user-doc requirement (parallel `SKL <Name>.md` + no-slash `# Name Discipline` H1) out of R-skill-md-05.
