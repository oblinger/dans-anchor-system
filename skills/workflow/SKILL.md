...
**The closing line — echo the status banner (the handoff made visible).** The point of the handoff is to show the user where things stand, so a work-armed turn **ends by echoing the anchor's status banner as its last line** — `frontier groomed · Ready N · Questions M · Verify K` (or `all caught up · nothing open`), straight from `state groom-list`. The agent already holds the current counts from its final `state` call; if it didn't mutate, one `state groom-list` pull produces the line. **Pull once and echo — do not make each `state` mutation emit a banner** (a turn calls `state` many times; per-call banners are noise, not signal). The one closing line lets the user pick their own next move at a glance: questions piling up → answer a batch; lots Ready → press `'` and crank.

The prefix names the **grooming** state, never open work: `frontier groomed` means the frontier is fully planned — it does *not* mean "nothing to do" (Ready>0 is still open work). `all caught up` appears only when Ready+Questions+Verify are all zero. (Saying "all caught up · Ready 1" was the confusing wording this fixed.)

**When the agent has a recommended next move, it appends it to the closing line — in bold.** The counts tell the user *where things stand*; a recommendation tells them *what to do*, and the agent often knows. Chief case: when the only Ready item is **fresh-session-scale** — large, self-contained in its feature doc, and better executed by an unfatigued context than by cranking on this turn's accumulated one — the honest handoff is **`→ recommend: compact` (or `clear`) then start F<n> fresh**, naming the item. The user acts on that immediately (`/compact`, or `/clear` when the feature doc + its survey fully specify the work and nothing in the current context is worth keeping). Other recommendations fit the same slot: `→ answer the 3 questions`, `→ press ' to crank`. Omit the bold line when the plain counts already imply the obvious move.
