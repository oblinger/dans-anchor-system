...
**Answering covers `Verify K` too, and that is the half the agent most often hands over unearned.** The banner counts verifications beside questions and the user answers both the same way, but the two differ in a way this invariant depends on: a question genuinely needs the user, while a Verify row usually does not. **Before a handoff, the agent runs every check it can answer itself** — the [[DAS verification]] positioning test is one question, *would the user's answer be any better than mine?*, and when the answer sits in a file, a log, or a probe, the honest answer is no. Run it in the turn that does the work and record the result; the row never surfaces. What is left over for the user is only what needs a human faculty — taste, preference, ratification, or an observation they make in ordinary use — which is exactly what `--why-user` makes the agent name at the mint and what audit-q **C47** flags afterwards on a row phrased as a machine event. **A Verify row the agent could have run itself is the same failure as a chat-only question**, arriving from the other side: it takes a slot on the handoff without needing one, and the user pays for a check nobody ran. `Verify K` must read as *K things only you can judge*.
...
- **The render** (`queries-render.py`) renders the bracket as-is in the body — `**[Blocked]**`, `**[Blocked F015]**`, `**[Waiting]**`, `**[Waiting 1d]**`, `**[Watching]**`, `**[Watching 7d]**` — and counts the row under its **horizon H2** (Now/Next/Later). **F305 gave all three a zone-3 count**: `[Blocked …]` folds into `Parked` alongside `[Verify]`, and `[Waiting …]`/`[Watching …]` fold into `Waiting`. Before that they contributed to *no* count anywhere and were invisible outside the raw backlog file — 33 such rows on TINK alone, 42 on MUX. They still contribute *zero* to zone 1's `Ready` / `User` buckets and *zero* to the TAG cascade's U or A, which is the point: counted, but not pushed at you. **All three states are reconsidered every `/groom` pass** — that's the reconsideration's primary purpose. An anchor whose only items are Blocked / Waiting / Watching falls through to TAG `[G]` (groomable — user/time/soak needs to clear something) or `[]` (nothing actionable).
