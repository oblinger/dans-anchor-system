...
**🚨 To swap a thing out, the mask must cover 100% of it.** This is the single most useful rule in this file. Fill has to make what it paints *continuous with what it is forbidden to touch* — so any surviving fragment of the old thing does not merely remain as a stripe, it **drags the replacement back toward itself**. Masking most of a mint shirt and asking for dark blue returned a dark blue body with a mint collar, because the unmasked collar was a constraint the model had to satisfy. Extending the mask to the garment's seam fixed it in one pass. **When a replacement comes back half-applied or colour-shifted, do not describe it harder — go and find the fragment you left outside the mask.** *(Dan, 2026-08-02; IMGEN009 Batches 5→6.)*

**Describe the masked patch as a photograph of the whole subject, not as an anatomical fragment.** A fill prompt opening *"The corner of a mans mouth with a fat cigar clamped in it…"* returned a **solid black blob** the exact shape of the mask, four times out of four. The identical region, same mask, same intent, reworded as *"A weathered man with grey stubble around his mouth, a short thick cigar stub held between his closed lips… photographic, evenly lit"* filled correctly on the first try. Fill still thinks it is painting a picture of a person; hand it a body part in isolation and it can return nothing at all. **A black patch is this failure, not a masking error** — check the wording before re-cutting the mask. *(Dan 2026-08-30; IMGEN010 Batches 13→14.)*

**Ellipse coordinates are the SOURCE image's own pixels, and both axes are scaled independently.** They were not always: `build_mask` used one factor, `size/width`, for x, y, rx and ry alike, so on a tall source every mask slid **down** by height/width — a 768x1024 portrait put the region 1.33x too low, landing a mouth mask on the collar. It went unseen because every earlier roll masked a square image, where the two factors are equal. Fixed 2026-08-30. The lesson outlives the bug: **always look at the green preview before spending on the fill**, and when a mask lands somewhere unaccountable, suspect the transform before your own arithmetic. *(IMGEN010 Batch 13.)*

**Some regions need several ellipses, not a bigger one.** A face sits *between* its own shoulders, so any single ellipse low enough to clear the chin also misses the shoulder line, and one high enough to take the shoulders eats the jaw. A body ellipse plus one small ellipse per shoulder plus one at the collar solves it exactly — neither side ellipse ever reaches the face's column. `--ellipse` takes several groups separated by `;` and unions them.
