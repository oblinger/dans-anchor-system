...
**Check the day's addresses, not just its times.** An event whose location is *correct* can still send Dan to the wrong place: Calendar's tappable address stops at the first newline, so a venue name on its own line is not part of the tap, and what is left may be a street with no ZIP. On 2026-08-30 that put him on Nob Hill for a party in the Presidio, 3.3 miles away. Triage is two fields and no network — **an entry is suspect when the tappable span has no ZIP *and* the event carries no geo pin**; either one alone is survivable. Then geocode the span **cold, in an incognito window**, because Dan's own machines resolve it correctly from Recents and return a confident false pass. Full procedure, including the repair and who owns it when: [[AREC Calendar Address Check]].

**How loudly a failure is said depends on how close it is.** More than a week out, raise it **once**, decide with Dan what to do, and then leave it alone. **Inside a week, re-raise it every morning until it is resolved** — a broken address inside a week is not allowed to go quiet, and this is the one place a repeated item is correct rather than nagging. [[Sparks]] covers today and tomorrow from the other side, acting the moment new information arrives instead of waiting for a morning. **And a second check runs alongside the first**: does the address actually point at the venue? That one reports *contradiction only* — an address that cannot be confirmed is not a finding and is never raised.
...
**This read is now the ONLY channel by which an estate failure reaches Dan.** Ruled by him 2026-09-01: `ob_check`'s banner and full-screen paths are record-only, and every `ob_daily` critical was demoted — **the checking system never draws on his screen any more.** Nothing else will bring him a red. So the morning read stops being a courtesy and becomes the whole surface, and the judgement is Lumen's: *"unless you judge some situation to be critical and imminent… many of these are critical, but not imminent."* Critical **and** imminent earns an interruption; everything else is at most a briefing line; a red he has already been told about does not repeat daily. A log entry's `visibility` field records how loud the *poster* thought it was — weigh it, do not obey it.

**Backups are the sharpest case of that rule.** `restic_tier2`, `backup_check` and `restic_verify` were demoted the same day, after an unparseable full-screen *"restic_tier2 exited 137"*. Their failures now land in the health log at daily visibility and nowhere else. **When a backup line is failing in the report, say so in the brief — nothing else will.**

**Apple Health has its own threshold, tighter than the rest.** If `apple-health.freshness` shows the archive behind by even **one day**, tell Dan — do not wait for warn level. The line is his, verbatim: *"Tap the Auto Export widget to restart health data."* The Automations widget sits on his first home-screen page and one tap on its centre fires the sync — that tap **is** the fix, which is why the line names it rather than describing the problem. The iMessage companion (`~/ob/bin/apple_health_fix`) stays available but must **not** be auto-fired; the 22:00 `health_nudge` in `ob_daily` is the same-day backstop, unchanged.
...
**Rotate FIRST, then write.** The opening act of this step is to move whatever day the file currently holds into [[Lumen Day Archive]] — **verbatim**, above its newest entry, never rewritten and never summarised.

**Demote every heading by one level, not just the H1.** The live file's `# Lumen Day · <Day> …` becomes `## <Day> YYYY-MM-DD · W<nn>`, **and its `## Decisions` / `## Today` / … each become `###`** so they nest under the day rather than becoming siblings of it. Drop the orientation line, which describes the live file and not that day. Missing the inner demotion trips `R-progressive-02` and makes every section read as a separate archive entry — it happened on the first real rotation, 2026-09-01. Only then is today's H1 written. Rotating first rather than last means a run that dies halfway leaves one clean day in the file instead of two, and it is why the archive is append-only: a past briefing is a record of what Dan was told, so editing one falsifies the record.
...
- **No anchor stays unheard — scan the starved list before filling Decisions.** Measured 2026-08-05 ([[Lumen Backlog#^T021|LUMEN T021]]): [[Ash]], [[Boone]], [[Ember]] and [[Vector]] had **never once appeared** in any briefing across the whole run of [[Lumen Day]] and its archive, while Ash alone carried **12 user-gated questions, more than any other anchor**. The anchors that did appear were exactly the ones the user had been talking about — a rich-get-richer loop in which an agent working quietly can never earn a mention, so its queue grows without bound and its questions expire unseen. Ash's did: one carried a window whose first date (2026-07-28) passed unnoticed, and another had already been answered by dictation and was still sitting in the queue. **The rule: before writing Decisions, read every `[U]` / `[U+A]` banner in `Q.md`, not only the anchors already in mind. If a *personal* anchor has user-gated questions and has not appeared in the last seven briefings, one of its questions takes a Decisions slot.** The three-item cap does not move; this only changes which three.

**🚨 Work anchors are OUT OF SCOPE — [[SV]] above all, and this is a boundary, not a tuning knob.** The starve check runs over Lumen's own domain: personal, health, family, admin, career-of-Dan's-own. It does **not** reach into SportsVisio's engineering queue. Dan, 2026-08-31, asked the question that settles it — *"What rule do you have about SportsVisio? I don't understand why you have anything to do with SV"* — and then: *"this is not yours as well. Just drop that."*

**He was right that it contradicted something already written.** Lumen's own `CLAUDE.md` § Boundaries says it plainly: *"Lumen does not duplicate domain-agent tracking. If a work item belongs to [[Sonar]], [[SV]], [[MUX]], [[HA]], or another anchor, it lives in that anchor's backlog."* This rule, added later for a real problem, quietly overrode that — and the failure it produced was exactly the shape you would predict: an **`az storage blob list` against three Azure containers** was put in front of Dan in a personal morning briefing, three days running, as a decision he owed. It is not a decision, it is engineering work waiting on an agent with Azure access.

**The generalisation worth keeping:** starvation is a real failure and the fix is right, but *whose attention is being protected* is the thing the original measurement never asked. An anchor being unheard **by Lumen** is only a problem if Lumen is the surface it should reach Dan through. SV's is his working day. **Pick its cheapest question rather than its most important one** — a queue that has never been touched is far more often unblocked by a single quick answer than by the right one. [[Vector]] is the cautionary case: its one open question is the sitting that would populate [[Life North Star]], and that file's emptiness is what keeps the effort-weighting check above dormant. The loop starved the thing that feeds it.
...
**While the file is an empty scaffold this step is a no-op** — say nothing at all rather than noting its absence every morning. It was created 2026-08-04 with placement and cross-references only; the life-area sections are on [[Vector Backlog]]. **It activates by itself the moment Vector fills in the first H2 carrying a Level of effort** — no further wiring needed here.
...
3. **Open it — `open "$HOME/ob/kmr/SYS/Staff/Lumen/Lumen Day.md"`** so it is the active tab in Dan's Obsidian, where he reads it. **Nothing here pins anything.** Dan asked on 2026-09-04 for the briefing to be pinned; the pin is a **spine Pin row**, set once by `skills/workflow/scripts/pin` — `pin Lumen "[[Lumen Day]]"` — and it already stands on [[LUMEN]]. It is not a per-morning act, and Obsidian's tab-pin is a different mechanism that must not be used as a substitute: **the vault has exactly one pin.**
