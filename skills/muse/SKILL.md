...
Pipeline: strip quarantine → `_transcribe` → **offer the transcript to `MUSE_ROUTE_HANDLERS`** (a memo that opens by naming its destination — *"sleep log, took a quarter ambien"* — is filed there by the destination's own script at its capture time, gets `state: routed` + `routed_to:`, and skips everything after this point except the item file and the Log Muse line) → `_askAI` for title (tool-less LLM call, safe against prompt injection in transcript) → sanitize title to `[A-Za-z0-9 _-]` only → compute sequence letter (YYYY-MM-DD A/B/C by count of same-day items) → write item file to the permanent archive folder → prepend bullet to `Quick.md` → macOS notification → drop a pointer into Sparks's Inbox.

**Route handlers** (Dan, 2026-09-04): each handler owns its own spoken address and is called as `<handler> voice --captured "<YYYY-MM-DD HH:MM:SS>" "<transcript>"`; exit 0 = filed, 3 = not addressed to me, anything else = failed and the memo takes the normal path so nothing is lost. The list is the `MUSE_ROUTE_HANDLERS` array in `scripts/muse`; the first entry is `~/ob/kmr/Topic/MED/MED Data/bin/sleep-diary` (address *sleep log* / *sleep diary*). Tests: `scripts/test-route.sh` (stubbed) and `scripts/test-route-e2e.sh` (real whisper, real vault, self-cleaning).
