...
Pipeline: strip quarantine → `_transcribe` → **the oracle** (one tool-less `_askAI` call on `MUSE_ORACLE_MODEL`, Opus 5, that can only emit three labels: `VERDICT: safe|suspicious`, `TO: sleep-log|<roster name>|none`, `TITLE: …`) → **route on the labels**: safe → offer the transcript to `MUSE_ROUTE_HANDLERS` (a memo that opens by naming its destination — *"sleep log, took a quarter ambien"* — is filed there by the destination's own script at its capture time, `state: routed` + `routed_to:`, and skips Quick and Sparks); unclaimed and `TO` a roster name → a pointer into that agent's Inbox (`state: delivered` + `delivered_to:`, skips Quick and Sparks — delivery, not activation); unclaimed and `none` → the default path; `suspicious` → `state: flagged`, Quick gets a bare `⚠ [title](…)` link (never the text inline), Sparks gets it with `flag: suspicious`, no handler is offered it and nothing forwards it → sanitize title to `[A-Za-z0-9 _-]` only → compute sequence letter (YYYY-MM-DD A/B/C by count of same-day items) → write item file to the permanent archive folder (its frontmatter carries `oracle: <verdict> <to>`) → prepend bullet to `Quick.md` → macOS notification → drop a pointer into Sparks's Inbox. **Oracle down** (dead key, unparseable answer) records `oracle: unavailable` and degrades to the pre-oracle path — handlers by their own address, transcript-derived title, Quick + Sparks — so nothing is lost and the sleep log needs no API. Ruled by Dan 2026-09-04 (SEC F315): *"100% of the inputs should go through the oracle … then it gets routed based on naming … we're not using Sparks as a verification."*

**Route handlers** (Dan, 2026-09-04): each handler owns its own spoken address and is called as `<handler> voice --captured "<YYYY-MM-DD HH:MM:SS>" "<transcript>"`; exit 0 = filed, 3 = not addressed to me, anything else = failed and the memo takes the normal path so nothing is lost. The list is the `MUSE_ROUTE_HANDLERS` array in `scripts/muse`; the first entry is `~/ob/kmr/Topic/MED/MED Data/bin/sleep-diary` (address *sleep log* / *sleep diary*). A handler owns its own spoken address — the oracle's `sleep-log` label is a record, never the gate. The roster the oracle may name is `MUSE_ROSTER` (the Staff slugs; `state drop` must resolve each). Tests: `scripts/test-route.sh` (44 stubbed cases: routing, delivery, flagging, oracle down, oracle nonsense, failed delivery) and `scripts/test-route-e2e.sh` (real whisper, real oracle, real vault, self-cleaning).
...
**The machine side already screens it.** Every non-suppressed memo crosses the oracle (§ Ingest), a tool-less call whose whole output is three labels — a jailbreak can yield a wrong label, and a label cannot act. A `suspicious` verdict reaches Sparks flagged and is forwarded nowhere; a `safe` one is routed by name. That screen sits *in front of* every agent, so no agent has to be hardened by prompt against raw transcripts (Dan: *"you could try to jailbreak Sparks as he tries to look at a text to decide if it's safe"*).
...
| `MUSE_ORACLE_MODEL` | `claude-opus-5` | The oracle's model, passed to `_askAI --model` (≈23¢ per 100 short memos; Haiku ≈5¢, Sonnet 5 ≈9¢) |
| `MUSE_ROSTER` | `Ash Atticus Boone Ember Hermes Lumen Munger Presti Scout Sparks Tink Wells Winnie` | Names the oracle may return in `TO`; each must resolve for `state drop` |
