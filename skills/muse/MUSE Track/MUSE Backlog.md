...
- **T010 — Capture-time routing: step 12 in `scripts/muse` drops addressed memos into agent Inboxes** [Done] — → [[MUSE010 - Capture-time routing step 12 in scripts muse drops addressed memos into|T010]] — none — shipped and fixture-verified 2026-08-31; the next real memo is the natural exercise. ^T010

- **T003 — MUSE ingest is dead under launchd: every drop since 2026-07-21 was quarantined, not transcribed** [Done] — → [[MUSE003 - MUSE ingest is dead under launchd every drop since 2026-07-21 was|T003]] — reproduce the failure under `_trust` rather than under a login shell — run `/Users/oblinger/bin/_trust muse-sweep` directly and capture the real per-file error, which the current log swallows behind the "repeatedly-failing" skip. Then diff that environment against the working interactive one (PATH, whisper model path, ffmpeg location, TCC-visible paths) and fix the difference at its source. ^T003
