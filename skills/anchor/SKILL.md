...
description: >
  ...
  /anchor status (activity tracking), /anchor docs-audit (docs vs source),
...
The toolkit verb for anchor machinery. Overloaded by design (F234 Q1=A, 2026-07-14): actions pertain either to a *single anchor* (config, docs-audit) or to the *anchor system as a whole* (scan, status, install) — one umbrella covers both.
...
| `/anchor status` | `scripts/stat.py` | Activity status tracking across anchors |
