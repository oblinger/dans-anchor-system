...
- **Safari on haorui is shared state, and a tab there may be someone else's staged work.** This skill's own deliverable is a tab left sitting on a live button — so before opening anything, dump the full window/tab inventory (`repeat with w in windows / repeat with t in tabs of w`) and keep it; close **by captured window id**, never `close front window`, which can close a different window than the one you opened; then re-inventory and compare. Learned 2026-08-19, when reading the Amazon order history cost an eBay return tab that had been staged two days earlier and was waiting on Dan. Nothing was actually lost — the underlying return was still open — but with no before-picture it was impossible to prove, which is its own failure. **The one-tab rule applies to the comparison tabs this skill opened, not to tabs it found.**
...
**A deadline read off a page is a snapshot, not a fact — know what recomputes it.** Return windows are usually **N days from the delivery date the retailer currently believes**, so they move when the estimate is replaced by an actual delivery. eBay's T480 order read *returns through Sep 10* before delivery and *Sep 7* after, and the box simply arrived three days early; the sibling order still reads a window computed off a *scheduled* date it never updated. Carrying both readings with "no mechanism verified" meant acting on the safer number rather than the right one for eight days. **Re-read the live page and work out which date the window is counting from** — then the earlier reading stops being a mystery worth hedging against.
