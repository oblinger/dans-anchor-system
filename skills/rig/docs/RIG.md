...
rig [--cloud BACKEND] [--agent SLUG] COMMAND [ARGS]
  ...
  up    --idle MIN (watcher, default 60)  --no-watch (loud)
...
1. **Owner prefix, required in config.** `~/.config/rig/config.yaml` has `prefix:` (Dan's is `do-`); rig refuses to run without it — no default, no derivation from `$USER`, because a guessed prefix would make `gc` offer to destroy whatever the guess matched. Every rig it creates is named `<prefix><agent>-<local>` (`do-svp-h100`; `rig name h100` prints the canonical form for scripts), on Azure in a resource group of exactly that name (the `rig-` group prefix went the same day — the owner prefix already says whose it is), tagged `rig-name` as before. **The prefix is how rig knows what is its own to destroy**: `gc` and the frozen nag act only on rig-tagged boxes carrying the configured prefix; `reap` (stop, reversible) still covers any rig-tagged box; everything untagged is listed in parentheses and never touched. Boxes that predate the rule (`a2x-dev`) are still found by the name they carry — `rig up a2x-dev` restarts it rather than making `do-a2x-dev` — but are never `gc`'d, since the prefix is the only evidence of whose they are.
2. **Two expiries, both set by `rig up` by default, both cloud- or box-side so they fire with the laptop shut and the session gone.** *The lease* is the VM's own **auto-shutdown schedule** (`az vm auto-shutdown`, a DevTestLab schedule in the rig's group that *deallocates* at a UTC time of day; a Contributor can set it). Default `lease_hours: 8`, `--lease H`, `--forever` opts out loudly; `rig renew <name> [--hours H]` pushes it; `rig ls` shows it in a LEASE column, where **`NONE` on a running rig is the line to act on**; ceiling 24 h because the schedule is daily. *The watcher* is on the box: `rig up` gives the VM a system-assigned identity holding the custom role **`Rig Self-Stop`** (`virtualMachines/read` + `deallocate/action`, created on the subscription the first time, assigned scoped to the rig's own group), and installs `/usr/local/bin/rig-watch` under a systemd timer (every 5 min, `/var/log/rig-watch.log`). It runs the same probe `reap` does — GPU compute processes, 1-min load ≥ 0.2, tmux input on any user's server, a logged-in tty — with a boot grace of `idle_minutes`, and when all are quiet for `idle_minutes` (60; `--idle N`) it **deallocates itself** through the instance-metadata token: bash + curl + python3, no CLI, no secret. `--no-watch` opts out loudly. Writing role definitions/assignments needs Owner — Juan granted Dan a conditioned Owner on `sv-startups-01` on 2026-09-03 for this; a Contributor gets the lease only. *Down, never rm*, on both paths. GCP has neither (instance schedules and self-stop need IAM the project does not grant, and SV compute left GCP) — `up --cloud gcp` prints `NO LEASE` and `NO WATCHER` and means them.
