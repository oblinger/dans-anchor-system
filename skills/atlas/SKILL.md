...
| **ATL Slugs** | `~/ob/kmr/SYS/Atlas/ATL Slugs.md` | Master index — one row per named thing with three columns: `Slug` (hook URL), `Atlas` (block-link to the H2), `Summary` (one-phrase distillation). |
...
4. **Read** `~/ob/kmr/SYS/Atlas/ATL Slugs.md` and **insert** the matching row at the same alphabetical position.
...
1. **Glance** Atlas.md + ATL Slugs.md so the user can browse:
   ```bash
   open "$HOME/ob/kmr/SYS/Atlas/Atlas.md"
   open "$HOME/ob/kmr/SYS/Atlas/ATL Slugs.md"
   ```
