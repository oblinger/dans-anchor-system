...
|---|---|---|
| **Atlas** | `~/ob/kmr/SYS/Atlas/Atlas.md` | Alphabetical glossary of named things. Each entry = `## Name` H2 + paragraph + `([[slug]])` terminator. |

Atlas is the single, canonical router. The former `ATL Slugs.md` companion index was retired 2026-07-20 — its slug and summary columns were redundant with the Atlas entry itself. Any deep detail about a named thing lives at the target (the thing's own anchor page, its Brief, its `META` doc if structural), never in the Atlas entry itself.

## Actions

| Form | What it does |
|---|---|
| `/atlas add <name>` | Add a new entry for `<name>` in alphabetical position; glances the result. |
| `/atlas update <name>` | Refine an existing entry in place. |
| `/atlas` (bare) | Glance Atlas.md so the user can browse or edit by hand. |
...
Deep detail about how X works — operating rules, gotchas, invariants — belongs at the target: the anchor page's `Brief`, its `{X} Design/`, or `SYS/META/META {Kind}.md` when it's a structural pattern (Lists, Logs, Flow). Never inline that detail into the Atlas entry.
...
### 5. Update the letter-index row

The top of Atlas.md carries a per-letter jump table (one row per letter of the alphabet). When an add/rename/remove changes the entries for a letter, update that letter's row to keep the jump-table current. Rows contain `[[Atlas#Name\|Name]]` block-links.

## Runbook — `/atlas add <name>`

1. **Read** `~/ob/kmr/SYS/Atlas/Atlas.md` to locate the alphabetical insertion point (the H2 immediately following where `## <name>` would sit).
...
4. **Update** the letter-index row at the top so the new entry is reachable.
5. **Glance** Atlas.md so the user can verify:
   ```bash
   open "$HOME/ob/kmr/SYS/Atlas/Atlas.md"
   ```
6. Report the entry added in one short line.

**Note:** direct `Edit`/`Write` on `Atlas.md` is blocked by `R-pathguard-04`. The atlas skill mutates the file via Python subprocess (`Path.write_text`) — see prior working examples in session history.

## Runbook — `/atlas update <name>`

1. **Read** Atlas.md, locate the existing `## <name>` H2.
2. **Edit** the body in place via Python subprocess. Apply discipline § 1 + § 2 — almost always means cutting, rarely means adding.
3. **Glance** Atlas.md.
4. Report what changed in one short line.

## Runbook — bare `/atlas`

1. **Glance** Atlas.md so the user can browse:
   ```bash
   open "$HOME/ob/kmr/SYS/Atlas/Atlas.md"
   ```
...
- Slug-side catalog: `[[SLUG]]` (date-sorted, scanner-managed; complementary technical view)
- Anchor page: `[[ATL]]`
