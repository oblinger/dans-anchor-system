...
The TO-DOs + Overview are generated by the tool-less `derive_overview` API call (`VOX_OVERVIEW_MODEL`, default a capable summarization model). If the call fails for a non-credential reason, the document degrades to transcript-only. A **credential** failure (`authentication_error` from both the env key and the keys store) is deliberately loud: the note's header carries a `> ⚠️ vox:` callout naming it, so a broken key cannot survive unnoticed for days (T619, 2026-08-29). The `## Transcript` heading is always present, so the retention sweep's transcript-size guard is unaffected.
...
#    The script reads $ANTHROPIC_API_KEY from the environment, then falls back to
#    `~/ob/bin/keys get ANTHROPIC_API_KEY` when the env var is empty OR when the API
#    rejects it (authentication_error — a stale session key is the common case, T619).
#    Without any working key the filename falls back to a sanitized email subject,
#    the note carries a visible credential warning, and everything else still works.
```
