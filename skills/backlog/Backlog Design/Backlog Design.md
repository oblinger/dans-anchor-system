---
description: Design docs for the backlog discipline — Now/Next/Later horizons + workflow states. PRD, architecture, interface, principles.
---
# Backlog Design

| -[[Backlog Design]]- | → [[kmr]] → [[SYS]] → [[symlinks]] → [[backlog/Backlog]] → [Backlog Design](hook://p/Backlog%20Design)<br>: Design docs for the backlog discipline — Now/Next/Later horizons + workflow states. PRD, architecture, interface, principles. |
| --- | --- |
| [[Backlog PRD]] | product requirements |
| [[Backlog System Design]] | system architecture |
| [[Backlog Architecture]] | architecture doc |
| [[Backlog Interface]] | top-level interface contract |
| [[Backlog Decisions]] | invariants |
| [[Backlog Features\|Features]] | dated feature specs (F-numbered) |
