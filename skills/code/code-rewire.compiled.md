...
- [ ] `{slug} Files.md` exists inside Dev folder
...
- [ ] Standard `{slug}.md` anchor page — create only when another trait requires it (SKILL.md replaces it by default)
- [ ] `{slug} Docs/` folder — create only when another trait requires it
...
- [ ] Lives under `Skill Agent/{slug}/` — separate from the skill folder
- [ ] Has `{slug}.md` anchor page
- [ ] Has `{slug} Docs/{slug} Plan/` with planning docs
