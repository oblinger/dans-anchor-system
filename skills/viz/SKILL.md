...
|---|---|---|---|
| `/viz svg` | **Hand-written SVG via the Write tool — default for architecture diagrams and any figure where the agent wants full visual control.** SVG is just XML; the agent authors it directly. The `.svg` file IS the editable source (no companion file needed). Full control over color, font, layout, geometry. **Exception — SVGs exported from a same-basename `.excalidraw`: the `.excalidraw` is the source.** The user edits it directly in **ExcalidrawZ** (`open -a ExcalidrawZ "<file>.excalidraw"`), then the agent re-exports with `excalidraw_to_svg.py`; never hand-edit the exported SVG. | `.svg` source (editable XML) · optional `.png` via `rsvg-convert` | "svg", "hand-write an svg", "architecture diagram", "boxes and arrows with my own look" |
...
| `excalidraw` | `.excalidraw` source + `.svg` | `.svg` (native) · `.png` | Source is user-editable in **ExcalidrawZ** (Mac app) or Excalidraw web — after user edits, re-export via `excalidraw_to_svg.py`; SVG is small and infinitely scalable |
...
Expected output: `Detected font: Inter` (if Inter is installed); `Themes: ['light', 'dark', 'neutral']`; a clean chart in `/tmp/viz-smoke.png` with brand-neutral deep-blue accent (not Northwind red).
