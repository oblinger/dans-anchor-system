...
# What a generator stamps into its own output. This is proof; a sibling filename
# is only a convention, and the two disagree (see `generator_source`).
GENERATOR_SIGNATURES = (
    ("d2", "data-d2-version"),
    ("Excalidraw", "svg-source:excalidraw"),
    ("graphviz", "Generated by graphviz"),
    ("draw.io", "mxfile"),
)


def generator_source(f, head=None):
    """How we know this `.svg` is a build artifact, or None if it is authored.
    ...
    premise fails in both directions: the repair dies at the next export, and the
    layout was the generator's call (for `.d2` the ruleset says so outright —
    those moves belong to the deferred D2 Jiggle, as ELK directives). A finding
    there would be true and unactionable.

    Two grades of evidence, strongest first. A **signature in the file's own
    bytes** is proof. A **sibling source under the same basename** is the vault's
    figure convention, and only a convention: `Viz Bench/renders` names its files
    `{case}-{technique}`, so `05-svg.d2` sits beside a hand-authored `05-svg.svg`
    while being a different technique for the same case rather than its source.
    That directory is a comparison corpus of deliberately un-repaired specimens,
    so exempting it is the outcome we want anyway — but the returned reason says
    which evidence fired, so a reader can tell the proof from the convention.
    ...
    if head is None:
        try:
            ...
            head = ""
    for name, sig in GENERATOR_SIGNATURES:
        if sig in head:
            return f"generated by {name}"
    for ext in GENERATOR_SUFFIXES:
        if f.with_name(f.stem + ext).is_file():
            return f"exported from {f.stem + ext}"
    return None
...
def _issue_verdict(detect, target, anchor_root, empty_msg):
    why = generator_source(Path(target))
    if why is not None:
        return "pass", f"{why} — layout belongs to the source"
