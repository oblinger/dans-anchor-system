...
if [ "$REMOTE_UP" = yes ]; then
  ...
  # F159 — memory share + anchor-system config
  ...
  echo "$V" | grep -q '"anchor_system_present": true' && rec PASS T-cla-anchor-system "anchor-system config on remote" || rec FAIL T-cla-anchor-system "anchor-system config missing"
  # runtime gate
