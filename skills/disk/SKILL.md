...
- `--json` — emit one JSON object (`missing`, `relocated`, `unexplained`, `capacity`, `refresh`, `suppressed`) instead of the text report. Same underlying data either way — computed once, rendered twice.
...
3. **Read the four sections in order:**
   ...
   - **RELOCATED** — rows whose `M3 disposition` documents a move (`M`, or text containing "moved"/"relocated"). Suppressed out of (1) but never hidden — still listed with the disposition text.
...
4. **Exit code drives the response.** `0` = clean, nothing to do. `1` = direction (1) or (2) found something — surface it to the user before taking any action; this tool reports, it never deletes or moves anything itself. `2` = setup problem (drive not mounted, catalog unreadable, rsync failed) — fix the setup, don't reinterpret the output.
