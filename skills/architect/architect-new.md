...
- **Do read** feature docs (`{slug} Docs/{slug} Plan/{slug} Features/F*.md`), the anchor PRD (`{slug} Docs/{slug} Plan/{slug} PRD.md` if present), the backlog (`{slug} Backlog.md`) for active scope, and any vision / system-design docs in `{slug} Docs/{slug} Plan/`.
...
2. **Locate input docs.** Build the input set:
   - Feature docs: `{slug} Docs/{slug} Plan/{slug} Features/F*.md` (every F-doc, current and historical, since features carry intent).
   - PRD: `{slug} Docs/{slug} Plan/{slug} PRD.md` if present.
   - System design / vision: `{slug} Docs/{slug} Plan/{slug} System Design.md`, `{slug} Vision.md`, or similar.
   - Backlog horizons: `{slug} Backlog.md` (read `## Active` / `## Ready` / `## Now` / `## Next` only — `## Later` / `## Done` are de-prioritized for greenfield scoping).
...
# {slug} Architecture (greenfield draft)
...
4. **Naming conventions.** If features consistently use `{slug} <slug>` naming, that's a structural principle.
...
- **Does not produce module docs.** The module list is a flat enumeration; per-module docs are generated by other skills (per `[[FCT Module Doc]]`).
