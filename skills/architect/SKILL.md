...
description: >
  ...
  Architecture lives in `{slug} Design/` — a single `{slug} Architecture.md`
  by default, upgraded to a `{slug} Architecture/` folder-doc once it grows
...
```
{slug} Design/
├── {slug} Architecture.md                 ← DEFAULT: a single file (most anchors stop here)
└── {slug} Architecture/                   ← UPGRADED to a folder-doc once it grows subsystems
    ├── {slug} Architecture.md             ← the index (same basename as the folder)
    ├── {slug} Foo Arch.md                 ← simple subsystem: single file
    └── {slug} Bar Arch/                   ← complex subsystem: itself a folder-doc
        ├── {slug} Bar Arch.md             ← folder-doc index
        ├── {slug} <Module-1> Arch.md      ← per-module arch doc (only when needed)
        └── {slug} <Module-2> Arch.md
```

The two top rows are the same artifact in its two states — a one-document architecture is **just `{slug} Architecture.md`** in `{slug} Design/`; the `{slug} Architecture/` folder is created only on upgrade. Don't pre-create the folder.
...
Standard top-of-doc per F060 (YAML frontmatter + H1 + dispatch-table placeholder). The dispatch table links to every subsystem doc and to `{slug} Principles.md`. Below the dispatch:

- **Single-document systems** — the architecture is just `{slug} Architecture.md` (a single file in `{slug} Design/`); there is **no** `{slug} Architecture/` folder until subsystems appear. Don't pre-create it — the file→folder upgrade is link-transparent (§ Folder layout), so wait until a second document is actually warranted.
...
Single-subsystem systems use exactly this format inside `{slug} Architecture.md` itself — no separate subsystem file.
...
`/architect` enforces this during its comparison step (per § 4.4) — **orphans** (in `{slug} Dev/` but in no subsystem modules-table) and **duplicates** (in two) surface as proposals to the user.
...
2. Read `{slug} Dev.md` frontmatter and extract the `module_docs_audited:` date field (written by `/audit docs` at the end of every audit pass — see `[[audit-docs]]`).
...
4. Branch on the result:
   ...
   - **No `module_docs_audited:` field exists** in `{slug} Dev.md` frontmatter: module docs have never been audited under this discipline — recommend a docs refresh.

### 2. Read existing state

Read (in this order, each at most once per run):
1. `{slug} Architecture/{slug} Architecture.md` (if exists) + every `{slug} <Subsystem> Arch.md` sibling.
2. `{slug} Principles.md` (for cross-link continuity).
3. The index of `{slug} Dev/` module docs — `find` for `{slug} <Module>.md` files; do NOT read each one yet.
...
- **Orphan** — module exists in `{slug} Dev/` but appears in no subsystem modules-table (per § Module-to-subsystem invariant); propose assignment to a subsystem.
