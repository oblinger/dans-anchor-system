...
5. **Generate per-section review HTMLs** named `{slug} <new-date-with-suffix> s1.html`, `s2.html`, etc. Each HTML is a **review view of the entire section** — full prose of the new version, rendered as HTML, with inline `<del>` / `<ins>` markup wherever the text changed from the prior version, and optional `.comment` divs explaining each change. **Sections with no changes still get a full HTML** showing the section's prose with no markup; the user opens it to read and mark accepted (`?` → `+` or `✓`). Same convention as `[[md-track-changes]]` and F112's section-mode HTMLs — the HTML is for *reviewing the section*, not just for displaying deltas.
