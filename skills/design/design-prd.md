...
- **User Stories**: bare canonical sentence with no acceptance criteria and the product clearly has more behavior to spec = candidate for [[DAS Stories]] folder-form extraction.
...
Create `{anchor}/{slug} Design/{slug} PRD.md` (single-file form by default) with the standard section spine per [[DAS PRD]] § Standard section order:
...
`{anchor}/{slug} Design/{slug} PRD.md` (single-file form, default) or `{anchor}/{slug} Design/{slug} PRD/{slug} PRD.md` (folder form, when stories migrate to [[DAS Stories]]).
