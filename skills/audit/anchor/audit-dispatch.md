...
- Called by the [[DAS anchor-dag]] walk to fix a node's down-links + table form.
...
The mechanical work is done by `scripts/audit-dispatch.py` — it parses the current table, rebuilds it to the Masthead + Member-zone shape, and (the load-bearing guarantee) carries forward any curated link the rebuild would otherwise drop. **Report only — the script never writes (T204, Dan 2026-09-03).** You apply the proposal with the Edit tool, so every change to a dispatch table is reviewed. The retired `--fix` joined unlisted children with the cell delimiter and, at one child, put it in the name cell.

```bash
python3 ~/.claude/skills/audit/scripts/audit-dispatch.py "<anchor>"        # proposed table + summary
...
- **A linkless row of bolded prose is a section label and is kept** (T090) — `|  | **SKILL GROUPS** |` divides zones inside a table; only unbolded linkless rows are stubs and get dropped.
- **What "pointing at deleted children" means** (T088). A row goes when it has links and *every one of them* resolves to nothing — either struck through by the maintenance pass, or naming something that no longer exists. A row is left alone whenever the tool cannot judge it: one live link among dead ones keeps it (the live link is why it is still there), and so does any target the tool cannot follow — a markdown href, a `hook://` link outside the breadcrumb. Targets resolve against vault files *of any type* (`.pdf`, `.xlsx`, not just `.md`), vault folders, and the HookAnchor command table, because catalog pages like [[DIR]] and [[Agent Roster]] put their content inside the dispatch table and their links name commands rather than files. If the HookAnchor lookup is unavailable the drop disables itself entirely — a resolver that failed to load is not evidence of absence, and this fix deletes rows.
...
Mirrors [[DAS anchor-dag]]'s two-table report at single-anchor scope: **mechanical fixes applied silently** (missing links, breadcrumb, threshold, normalization); **semantic choices surfaced** (group categories, structural-vs-member ambiguity, stale-curated-link calls). Loop until the table is clean, same discipline as `/audit q`.
...
- [[DAS anchor-dag]] — the corpus-level discipline whose walk calls this per anchor ([[SYS Anchor DAG]] is this vault's application of it).
