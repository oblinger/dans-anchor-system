...
**Done** — executed 2026-07-01. `/lint` deleted (skill folder, SKL Lint doc, SKA lint anchor all retired); unique content folded into [[audit-structure]] § Mechanical scanner + [[audit-docs]] § Module-doc warning reference + [[DAS Audit]] § cab-audit; `cab-lint.py` renamed to `skills/audit/scripts/cab-audit.py` with `~/bin/cab-audit` symlink; vault-wide reference sweep complete (historical records left verbatim).

### Known residue (pre-existing bugs in cab-audit.py — fix lands with M-Migrate.2)

- **Exceptions-path mismatch** — ~~the script reads per-anchor exceptions from `.skl/lint/exceptions.md` while all docs (per F059) say `.anchor.d/lint/exceptions.md`~~. **Closed 2026-08-08 by deletion, not by reconciliation (TINK T398).** Neither path had ever held an exceptions file, so the whole mechanism had suppressed nothing since it was written. **Evidence sentence corrected the same day** — the closure originally read *"`.skl/lint/` appears zero times in the vault"*, which is false: that directory does exist, as `cab-audit.py`'s own on-demand `source-cache` (172 JSON entries mirroring the repo's `.py` files, created fresh at 17:12 on the day of the closure). The original scan searched markdown for *textual references* and reported zero, then stated the result as a claim about the filesystem — a clean scan being a claim about the scanner rather than about the corpus. What is actually true, and what the closure rests on, is narrower and checkable: **no `exceptions.md` has ever existed under `.skl/lint/`**, and `~/.claude/skills/cab/` — the second path — does not exist at all. The one live exception surface is now `{slug} Track/{slug} Exceptions.md`, read by `audit-plan.py` ([[R-exception-discipline]], [[Tink314 - Exceptions: a graded, user-approved escape from any checked rule|TINK F314]]).
