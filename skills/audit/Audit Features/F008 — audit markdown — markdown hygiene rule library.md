...
**Distribution** (Q4-a leaning): `anchor-system-init.sh` extends to merge the Stop hook entry into `~/.claude/settings.json` and seed `~/.config/anchor-system/audit-markdown/config.yaml` with default rule selection.
...
~/.config/anchor-system/audit-markdown/            # per F080 unified namespace
...
`compile-rules.py` walks `skills/audit/checks/rules/*.md` (bundled) and `~/.config/anchor-system/audit-markdown/rules/*.md` (user), extracts each ` ```python ... ``` ` block under each H2, and concatenates into `~/.config/anchor-system/audit-markdown/compiled/rules.py`. The compiled module imports a uniform `Finding` dataclass + helper utilities from a base module, then defines one function per H2.

Compile invocations:
- **On install** — `anchor-system-init.sh` runs it once after writing the config.
...
```bash
# ~/.claude/settings.json — hooks entry (added by anchor-system-init.sh)
"Stop": {
...
1. Read `~/.config/anchor-system/audit-markdown/config.yaml` to learn which rules are active.
...
```yaml
# ~/.config/anchor-system/audit-markdown/config.yaml
rules:
...
`scripts/anchor-system-init.sh` extends to:

1. Symlink `skills/audit/` (existing curated-skills behavior).
2. Run `audit-markdown compile` to produce initial `~/.config/anchor-system/audit-markdown/compiled/rules.py`.
3. Write seed `~/.config/anchor-system/audit-markdown/config.yaml` with all bundled rules enabled (only if file doesn't exist — don't overwrite a user's customization on re-run).
...
- **Stop hook auto-registration** (Q4 (a)) — needs `anchor-system-init.sh` to merge the Stop-hook entry into `~/.claude/settings.json`. Until then, users invoke manually.
...
- **Q4 — Team-install path — how does this dovetail with F080 + SKA Deployment Arch?** — **Resolution: (a) Auto-register via `anchor-system-init.sh`** — init.sh installs the skill, adds the Stop hook entry to `~/.claude/settings.json` (merged, idempotent), and seeds `~/.config/anchor-system/audit-markdown/config.yaml` (per F080 unified namespace) with default rule selection. Turnkey for team-mates. Incorporated into Design § Install.
...
- **Q6 — Where do rule files live (both bundled and user-added)?** — **Resolution: (a) Bundled in `skills/audit/checks/rules/*.md`; user-added in `~/.config/anchor-system/audit-markdown/rules/*.md`** — read-only bundled + mutable user-space is the standard CLI convention. Compile step reads both directories. User disables bundled rules via config selection. Incorporated into Design § Rule file locations.
