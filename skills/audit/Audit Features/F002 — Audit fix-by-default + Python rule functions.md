...
- **Distill → a pre-computed merged module.** The **`/distill`** skill compiles **all** the document/anchor rule bodies that exist across the rule sets into **special merged Python module(s) in the config folder** (e.g. `~/.config/anchor-system/…`). A distill run gathers every rule's Python and merges the bodies together, so at runtime there is already a precomputed module that applies all applicable rules in quick succession — no per-write discovery/parse cost.
