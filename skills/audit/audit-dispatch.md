...
- **What "pointing at deleted children" means** (T088). A row goes when it has links and *every one of them* resolves to nothing — either struck through by the maintenance pass, or naming something that no longer exists. A row is left alone whenever the tool cannot judge it: one live link among dead ones keeps it (the live link is why it is still there), and so does any target the tool cannot follow — a markdown href, a `hook://` link outside the breadcrumb. Targets resolve against vault files *of any type* (`.pdf`, `.xlsx`, not just `.md`), vault folders, and the HookAnchor command table, because catalog pages like [[DIR]] and [[AGENT]] put their content inside the dispatch table and their links name commands rather than files. If the HookAnchor lookup is unavailable the drop disables itself entirely — a resolver that failed to load is not evidence of absence, and this fix deletes rows.
