...
Takes a single markdown document and checks it against the **`R-doc` umbrella** — the doc-level facet rulesets (markdown hygiene, file-association, ruleset form, brief, discussion, log, messages). The doc-mode sibling of [[audit-anchor|/audit anchor]]: same [[F001 — Rule-driven audit engine — resolve, run, judge|F001]] engine, smaller facet set, targets are *regions within the file* rather than files in a tree.
...
- Umbrella: `R-doc` (see the doc-facet rulesets under `rulesets/`).
