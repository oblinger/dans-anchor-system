...
`{slug} Docs/{slug} Dev/{slug} Files.md` (if present) must follow [[DAS All Files]]. Common mistakes that need to be caught:
...
| Source files use `→ [[doc]]` arrows | Grep source-file lines for `→ \[\[` | Per [[DAS All Files]], source files use filename-as-link: `[[OBU Lib\|lib.rs]]`. Arrows are only for non-source files pointing to external specs |
