...
5. **100% of warnings must go to zero on every audit pass.** No "user input needed" carve-out, no QFix catalogue of unfixable residuals. The discipline (per user direction 2026-06-04, replacing the prior "mechanical-only never agent-guessed" rule): **every C-code on the audit's surface has an agent-side fix path; the agent's job is to take it.**
   ...
   - **C44 / C45 — [Questions] task→doc assertions (F233)** — flag-only, never auto-fixed (correctness is the authoring skills' job; Warden/audit catches lapses, never papers over them). C44: add the missing → `[[F<n> — Title]]` link (or inline Q sub-bullets on a T-/B-row). C45: relocate the '## Open Questions' block to the first H2 below the H1 (run `state revalidate <anchor> <doc>`); migrate any already-resolved Qs to the bottom '## Resolved' H2 while you're in the file.
