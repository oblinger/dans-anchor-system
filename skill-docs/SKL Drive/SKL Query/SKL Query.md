...
`/query` is the universal asking subroutine — it routes a decision the agent can't make alone into a question you can answer from what's written, and eliminates every question it can before that. It is the **query half of the resolution layer** (with [[DAS Groom]] as the backlog half).
