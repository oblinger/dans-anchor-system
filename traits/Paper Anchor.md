...
```
{Paper Name}/
├── {Paper Name}.md                  marker file → (See {slug}.md)
├── {slug}.md                        anchor page (link table + version table)
├── {slug} YYYY-MM-DD.md             merged version (one per version date)
├── {slug} YYYY-MM-DD s1.html        section 1 revisions (track changes)
├── {slug} YYYY-MM-DD s2.html        section 2 revisions
├── ...
└── {slug} Research/                 optional: research materials
```
...
| --- | --- | --- |
| [[{slug} YYYY-MM-DD]] | s1 s2 s3 ... |  |
| [[{slug} YYYY-MM-DD]] | (original) | s1: Intro, s2: Methods, ... |
...
- Edited sections: link to HTML with track changes (`[[{slug} YYYY-MM-DD s1.html|s1]]✓`)
...
3. Claude creates/updates `{slug} {DATE} s{N}.html` with track changes
...
2. Create marker: `{Paper Name}.md` → `(See [[{slug}]])`
3. Create anchor page: `{slug}.md` with link table and version table
...
Anchor-page kinds catalog: [[DAS Anchor Page]]. Synthetic example: [[HWP]]; real instances: [[ABP]], [[ASP]].

*(Maintainer note — this is the trait spec for every Paper Anchor: only trait-level rules that hold for all Paper anchors belong here; per-paper section names, review cadences, and one-off conventions live on that paper's own anchor page, never here. Changes to the version-table schema, the `s1`/`s2b` section-numbering scheme, the `✓` completion marker, the row-1-working / row-2-source orientation, or the "paper flow review" / "paper flow merge" trigger phrases are a migration, not a tweak — re-check downstream skills and `/audit` for existing Paper anchors, and don't regress those conventions. Umbrella catalog: [[DAS Traits]].)*
