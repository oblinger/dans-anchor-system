...
```sh
state -a <anchor> Backlog <F<n>|T<n>|C<n>|Q<n>> <define|set|resolve|remove> [flags]
state -a <anchor> "<doc path>" <Q<n>|V<n>> <define|resolve|remove> [flags]
```

Common flags: `--status`, `--horizon`, `--next`, `--verify`, `--body`, `--title`, `--choice`. Use `F+` / `T+` / `Q+` to mint the next number rather than picking one.
