...
where:: `group: file, folder, slot`
description:: The rules for authoring a facet — what every facet spec doc (a `DAS <Name>.md`) must contain and conform to. **Distinct from the umbrella [[R-facet]]**, which aggregates each materialized facet's *own* embedded rules (so an anchor's `{slug} Backlog.md`, `{slug} Testing.md`, … get audited); `R-facet-spec` instead governs the **facet-spec documents themselves**. **The target set is chosen by the spec's own `group:` declaration** — `file`, `folder` or `slot` — never by filename or folder, per [[DAS Facet]] § Facet groups (*"the group is declared in the spec, never encoded in which folder the spec lives in"*). A `group: discipline` spec selects nothing of its own and is out of scope here by construction. **This replaced a 48-clause hand-maintained negative** (a `DAS *.md` glob minus a 21-name brace expression plus 27 `!DAS <name>.md` clauses), every clause of which had been appended by an agent after a finding fired on a document the list had not yet heard of — [[Tink Backlog#^T361|T361]], deleted 2026-08-11.

**The swap was not finding-neutral, and the difference is the result.** Across `facets/` the finding set is **identical** — 36 fails before and after — so the negative was fully derivable from the positive *there*. What changed is membership: five specs living in `facets/` declare `group: discipline` and correctly drop out ([[DAS Doc Structure]], [[DAS Naming]], [[DAS Common Testing Types]], [[DAS Move]], [[DAS Template Variables]]), while **six specs living in `disciplines/` are slot or folder facets and are now audited for the first time** ([[DAS spine]], [[DAS heart]], [[DAS orientation-line]], [[DAS electric-zone]], [[DAS stream]], [[DAS file-association]]). The old list was keyed on filename, so it could never have reached them. Their 19 findings are true positives, not regressions — they are carried on T197 Q2 because the largest of them (`-02`, H1 form) is a rename of six published documents.

**Known residual risk, stated rather than hidden.** A brand-new spec that forgets `group:` is silently out of scope — under-inclusion, which reads as a clean green. A misspelled *value* is refused loudly (the selector raises), but absence cannot be distinguished from "this is an index page, not a spec" without reintroducing a list. The old failure was loud and this one is quiet; that is the trade the positive definition costs.
...
A facet spec must not state a **retired** anchor location as though it were current. The one retired token today is **`{slug} Docs/`**, withdrawn 2026-08-05 ([[Tink Backlog#^T514|T514]]). Its replacements are not a rename: `Docs/{slug} Plan/` and `Docs/{slug} Design/` both collapse into **`{slug} Design/`**, `Docs/{slug} Dev/` becomes **`{slug} Dev Docs/`**, and `{slug} Outputs/` moves under **`{slug} Track/`**.
