...
The Outputs folder lives at `{slug} Track/{slug} Outputs/`, not at the anchor root or elsewhere. Outputs are produced work, so they file with tracking rather than with design — which is where the live instances already sat when `{slug} Docs/` was retired ([[TINK Backlog#^T118|T118]]).
