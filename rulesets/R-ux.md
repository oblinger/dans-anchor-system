# RULESET R-ux
include::
where:: `file:{anchor}/**/* UX Design.md, !**/DAS *.md`
exclusion-note:: `!**/DAS *.md` exempts the facet-spec catalog (a `DAS <Name>.md` is the SPEC for the facet, not an instance; specs are governed by [[R-facet-spec]]) — the same exclusion [[R-prd]] carries, and load-bearing here because `DAS UX Design.md` is itself a `* UX Design.md` filename.
description:: facet spec for the human user-facing surface doc

> **Selector added 2026-08-11 ([[Tink Backlog#^T349|T349]]).** This set declared no `where::`, so it inherited `always` — every file in every anchor — while its rules describe one document kind. [[DAS UX Design]] names that document unambiguously: `{slug} Design/{slug} UX Design.md`, single-file, with an anchor-folder upgrade form. The selector globs the basename so both forms match, and carries [[R-prd]]'s `!**/DAS *.md` exclusion because `DAS UX Design.md` would otherwise select itself as an instance of the facet it specifies. Measured before writing: **8 instance(s)** — eight live instances across MUX, DMUX, DIV, SKD, SVW, OBU, Forum and the HBR gallery — a real corpus, and small enough that arming this set is a measurable step rather than a leap. Selector, not arming: [[R-facet]] is still inert ([[Tink Backlog#^T208|T208]]).
