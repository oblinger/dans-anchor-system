...
A dispatch table is the **distilled set of jump-destinations**, not an explanation of them. No meta-discussion of what a link *means* belongs in a cell. **Ideally none at all**; where a modifier is genuinely load-bearing, **at most two words in parentheses** as an adjective. A link's meaning belongs on the linked page itself — its top line (H1 + first sentence) and its `description` frontmatter — not in the table that points at it. Dan, 2026-08-22, on the cap: *"ideally 0 words. But if modifiers are critical, you can add them, but it can't be more than two words."*
...
**Check pattern:** every right cell of the doc's own masthead, after the identity row and the GFM header separator, up to (not including) the first electric-marker row — strip every wiki-link and markdown link, then every parenthetical of ≤2 words, then every code span; any word character left over → fail.
...
**Since the same day this rule also has a hard-DENY twin, [[R-dispatch-guard]]** — the `tool:pre:Write`/`tool:pre:Edit` (and best-effort `tool:pre:Bash`) veto that refuses a write introducing a NEW offending cell before the bytes land, sharing this rule's exact checker core (`masthead_narrative_offenders`) and its exception escape. This doc-rule remains the post-write surface: it names the law, sweeps the legacy corpus, and catches what the pre-hook cannot see.
