...
**A described group label is exempt — `| **Registers** | *entity-keyed: you arrive knowing an identity and look it up* |`.** A group heading in the spine may carry its gloss, and only when it really is one: the left cell is a non-link label and the right cell is *entirely* one italic span; any prose outside the italics is narrative and counts. Dan, 2026-08-29 ([[Tink Backlog#^T623|T623]] Q4): *"if it really is a group label, then it absolutely makes sense to have words on that"* — with the worry that it could be abused, which is why the shape is narrow. Keep the gloss to a single line; that is a preference he chose not to make a rule (*"maybe that's just a recommendation, not a harsh rule"*), so the checker does not count lines.
...
**Suspended to `warn` for one day, 2026-08-29, and re-armed to `fail` the same evening ([[Tink623 - R-dispatch-table-06 six days on 434 of 1,604 mastheads (27%) violate|TINK T623]]).** Six days of `fail` + deny had measured 434 of 1,604 mastheads violating, and on the worst offenders — the SV engagement cards, the person summaries, the SONAR company cards — the "narrative" was the page's own substance with nowhere legal to go. The day at `warn` was spent giving it a place: a link-left row is the child's description (exempt, above), a group label may carry an italic gloss (exempt, above), and a page's own facts live in a `| Card |` heart under the H1 ([[DAS heart]] § Fact card). 66 pages were flipped link-left, 60 had multi-link cells split one row per link in the Rolodex grouped form, and ~90 pages had their fact rows moved into a card, every write proof-gated (link multiset and word multiset unchanged per page). The evening measurement: **0 of 1,541** mastheads offend. [[R-dispatch-guard]]'s four denies returned the same evening.
...
### RULE R-dispatch-table-07 — An anchor page's dispatch table ends with a catch-all marker; no other page carries one (checked)
mend:: dispatch-rebuild
**The rule, cut and dried (Dan, 2026-08-29):** *if you are an anchor page — the page that fronts a folder — you must have a spine, and your spine must have an electric section. Otherwise you may not have an electric section, and you may or may not have a spine.* The marker enumerates the files the page **owns**; a page that owns no files has nothing for the machine to list, and a marker there lists someone else's children — measured the same day: [[DAS ask-format]] carried a `...` for twenty minutes and the rebuild filled it with every sibling in `disciplines/`, the folder [[disciplines]] already fronts. A non-anchor page may still carry a masthead when its related work is worth routing to; it just ends at its last hand row.
...
**Check pattern:** on a page that fronts a folder (the folder-note beside a `.anchor`, or the folder's own name-note), the table's final row is `...`, `| --- | |`, a trailing `+`-group row, or an electric separator (`---`/`^^^`/`+++`) followed only by its auto-emitted member rows. On a page that fronts no folder, **no** electric marker is present. (The second half is stated 2026-08-29 and not yet mechanically checked — `S07` covers the first half in `spine_check.py`.)
