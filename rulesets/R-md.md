...
A literal `<` or `>` may appear **only** inside an inline code span or a fenced code block. Everywhere else — prose, tables, headings — it must be backticked (`` `<slug>` ``) or HTML-escaped (`&lt;` / `&gt;`). This tightens R-md-01: the whitespace-padded form `< x >` is **no longer** acceptable, because it still renders as a stray bracket and several renderers still try to parse it as a tag.
...
**Why:** raw angle brackets outside code silently become (broken) HTML in Obsidian + CommonMark — a `<slug>` placeholder vanishes from the rendered view with no error. Forcing backticks makes every such token both render-safe and visibly code.
