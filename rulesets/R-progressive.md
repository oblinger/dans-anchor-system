...
**The summary region is the TOC table or the dispatch masthead — NOT the prose under a `## Summary` heading.** The hash is taken over exactly the rows that open `| [[`, `| **[[`, `| -[[` or `| →`, so rewriting `## Summary` re-blesses nothing and leaves the flag standing. This is stated here because the rule's own message used to invite that reading and two agents in a row took it, rewriting prose the checker never looks at.

**When a section is added or removed, the actionable repair is the TOC, and the message says so.** A content-outline TOC is *derived* from the section set, so the moment a `##` appears or disappears the TOC is stale by construction — it is still listing a heading that is gone, which is a dead `[[#…]]` row a reader can click. So an add/remove emits the generator command (`md-toc.py`) rather than the generic re-read, and the rows are never hand-edited — they carry figure spaces.

**Two gates, and the second was found by measuring rather than by reasoning.** The message is the TOC one only when the summary region actually contains `[[#…]]` rows **and** the doc is **file-scope**. A *masthead*-backed page fails the first: its rows name sibling files and survive section churn untouched. A **container** page fails the second, and this is the one that is easy to get wrong — its units are its **member files**, so what drifted is its sibling set and regenerating its outline repairs nothing. Every backlog in the vault is exactly that shape, a namesake page over a folder that *also* carries a content TOC, so a fix without the scope gate sends each of them to the generator for a change the generator cannot see. Measured on the live registry 2026-08-20: of **12** documents standing flagged, only **4** are file-scope at all, and **3 of those 4** carry the dead row — the other 8 correctly get the generic message.

**Check pattern:** against the blessing registry (`~/.warden/disclosure.json`), changed-units ÷ total ≥ 0.25, or ≥1 unit added or removed.

**Anti-nag, and the key it turns on.** Having prompted on a drift, the rule stays quiet until either the summary region is rewritten or the drift *grows*. The suppression key must match what fired: a **content-fraction** prompt keys on the full `{name: hash}` map, because there "it moved further" is precisely the re-prompt condition; an **added/removed** prompt keys on the **names alone**, because keying it on content hashes lets any unrelated edit anywhere in the document mint a fresh key and re-fire the same unanswered prompt on every single write. That was the state until 2026-08-20 — a key finer than its trigger, which is the same defect as no key at all.
