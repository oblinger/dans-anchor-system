...
> **This set declares no `where::`, and it is the clearest case that the grammar is what's missing — 2026-08-11 ([[Tink Backlog#^T350|T350]] / [[Tink Backlog#^T349|T349]]).** *"Applies when an anchor builds a macOS `.app` bundle"* is a condition about the anchor, not about a path, and the four available selector kinds (`always`, glob, `anchor`, `sentinel:`) cannot express it — so all five rules, every one `(checked)`, inherit `always`. T212 measured what that costs the moment [[R-code]] is armed: TINK's judgment manifest went **984 → 1,124**, 140 new LLM calls on a documentation anchor, from these five rules asking whether it ad-hoc-signs a macOS app it does not have. Filed with the other nine trait-gated sets as [[Tink Backlog#^T350|T350]].
