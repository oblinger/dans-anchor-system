...
>
> **One thing surfaced while fixing these and is deliberately NOT fixed here.** [[R-dispatch-table]] declares its selector in **YAML frontmatter** (`where: "file: {anchor}/**/*.md"`) rather than as a body `where::` line — so `parse_ruleset_block` reads `where=None` and all **15** of its rules fall through to the `always` default. It is also a standalone `R-*.md` carrying frontmatter, which `R-ruleset-11` forbids; `chk_ruleset_no_frontmatter` misses it because the guard only fires when *the file's first non-blank line is `# RULESET`*, and here that line is the frontmatter's own `---`. A guard that skips exactly the files it exists to catch. Moving the selector into the body would rescope 15 live rules, so it wants its own measurement rather than a quiet edit on the way past.
