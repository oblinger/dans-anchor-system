...
> **The Check pattern above was unsatisfiable as originally written, and that is why the rule caught nothing for as long as it existed ([[TINK Backlog#^T217|T217]], 2026-08-11).** It read *"if the file's first non-blank line is `# RULESET`, assert no `---` frontmatter precedes it"* — but nothing can precede a file's first non-blank line, so the guard's own entry condition excluded every violation. `chk_ruleset_no_frontmatter` implemented it literally, judged a frontmatter-bearing file by its opening `---`, and answered `('pass', 'not a standalone ruleset file')` about files named `R-*.md` sitting in `rulesets/`. Deciding kind on the body instead is the repair, and it was measured before landing: across all **122** vault files carrying a `# RULESET R-` block, exactly **one** verdict changes — [[R-fex-manifest]], the worked example of this very format, which carried frontmatter duplicating its own `description::`. Fixed in the same pass, so the rule is back to zero findings honestly rather than vacuously.
