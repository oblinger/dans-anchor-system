...
Ruleset for the Agenda facet — spec: [[DAS Agenda]]. **Not armed.** [[R-facet]] names it, and that umbrella is outside the `R-doc`/`R-anchor` closure `audit-plan.py` resolves, so no rule in this set has ever entered a plan (measured 2026-08-11, [[TINK Backlog#^T208|T208]]). Arming it means naming it in [[R-doc]] or [[R-anchor]] — and measuring the blast radius before doing so, per [[R-doc]]'s own record of what activating a dormant set costs. A second, sharper problem here: `R-facet`'s `include::` does not name `R-agenda` at all, so even the catalog claim this line makes is false. Sibling of [[R-backlog]] / [[R-status]] in the Tracking group.
