...
**Only a `fail` is suppressed — a `warn` is not**, and this rule used to say *"the finding"* with no severity carve-out, so a row aimed at a warning read as perfectly valid and did nothing. Stated here 2026-08-11 because leaving it implicit cost a real acceptance: [[ATT|Atticus]] wrote a well-formed `A` row against `R-spine-07` on `Staff/Atticus/Atticus.md` — a deviation Dan had personally graded — watched the engine report it as *stale*, and withdrew it rather than leave a row that reads as coverage while suppressing nothing. That acceptance now survives only in the file's Log, where no instrument reads it. The **misdiagnosis** is fixed (`test-t201-warn-exception-diagnosis.py`): such a row is now reported as *fired, at a severity this table cannot suppress*, never as stale. **Whether `A`–`C` should suppress warnings too is [[TINK Backlog#^T201|T201]] and is Dan's call**, since widening it silently changes what every existing `A` row suppresses in every anchor. Note the pull: the warning tier is where the judgment calls live (`R-rocks-05`, `R-spine-07`, `R-rocks-04`'s expansion half), so it is exactly where accepted deviations want to cluster.
