...
> **All seven sub-sets now declare the same selector, 2026-08-11 ([[TINK Backlog#^T349|T349]]) — four of them did not before, and the discrepancy was silent.** [[TINK Backlog#^T349|T349]] measured that **17 of the 37 dormant sets declare no `where::` at all**, and a set without one inherits `always` — every file in every anchor. Four of those seventeen were here: [[R-svg-hygiene]], [[R-sugiyama]], [[R-tufte-data-ink]] and [[R-bringhurst-typography]], each carrying rules that read a `<text>` element or an SVG `viewBox` and would therefore have answered `error` — *the checker malfunctioned* — on every markdown file in the vault the moment this umbrella was armed. They now carry `{anchor}/**/*.svg`, the form [[R-svg-jiggle]], [[R-diagram-geometry]] and [[R-c4]] already used.
>
> **A fifth spelling was quietly reaching less than half the corpus.** [[R-wcag-contrast]] declared `file:*.svg`, which after `_glob_to_relpattern` is a pattern with no `**` — so it matches only SVGs sitting **directly at an anchor root**. Measured over all 127 vault SVGs across 38 anchor roots: `file:*.svg` selects **69**, the sibling form selects **127**. The accessibility set — text contrast, colorblind readability, arguably the one whose findings a reader cannot work around — was scoped to 54% of the diagrams while every set beside it saw all of them, and nothing anywhere said so, because a narrow selector reports a clean green over the files it does reach. Normalized to the sibling form.
>
> Both fixes are preconditions rather than arming: this umbrella is still inert ([[TINK Backlog#^T208|T208]]), and T212's finding is that arming a set before it has a selector converts a silent zero into a loud recurring bill.
...
- [[DAS Ruleset]] — RULESET format spec.
