...
> **This umbrella is not armed, and "adopt `R-git`" is not a thing anyone can do.** `audit-plan.py` resolves a fixed umbrella — [[R-doc]] in doc mode, [[R-anchor]] in anchor mode — and nothing reads a per-anchor ruleset declaration, so a set reachable only through here never loads. See [[TINK Backlog#^T208|T208]] for the general case and [[TINK Backlog#^T212|T212]] for the ten umbrellas in this position.
>
