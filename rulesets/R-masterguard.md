...
> [!info] Provenance
> Commissioned as [[Atticus Backlog#^T181|T181]], split out of [[Atticus Backlog#^T178|T178]] when the write-once rule was ratified 2026-08-13. The rule was written down that day ([[Disk Conventions]] § The write-once rule, [[Disk Procedures]] § P5) and half-instrumented: `master_sweep` catches orphan scratch before a clone can make it permanent, which is the *completeness* half. This is the *immutability* half. Until it existed, an agent could copy over an existing master path and nothing would notice — while the ledger went on asserting the old checksum, which is worse than making no claim at all. [[Atticus Backlog#^T147|T147]] and [[Atticus Backlog#^T180|T180]] are the same lesson twice: an unenforced claim reports success against a weaker test than the one specified, and a discipline with no mechanism decays.
