...
- **A flat document is exempt at any length — fewer than 4 headings.** Length was never the whole trigger; the Check pattern below has always said *"heading count + body lines"*, and only the body-lines half was implemented. The H1 is not a TOC row, so a doc with three headings would carry a two-row table: friction with no navigation value, and the opposite of this rule's own rationale. Added 2026-08-20 ([[TINK Backlog#^T555|TINK T555]]), reported from `Log/VOX/`, where a `type: vox-transcript` doc has an H1 and a `## Transcript` and nothing else. **The corpus had already voted** — `2026-08-15 Juan — game break signals and metrics.md` is 1,072 lines with no TOC and `2026-08-10 Lewis on Singapore.md` is 6,631 — so the rule had been firing and being ignored, the failure mode this discipline names. Measured before the number was chosen: 381 docs fire today, a `< 4` gate silences **40** and leaves **341**, and both transcripts carry exactly **3** headings, so 4 is the smallest threshold that answers the report rather than a round number. Preferred over a `type: vox-transcript` frontmatter exemption because it is the general statement of the same fact and covers every other flat-but-long doc at the same time.

**Check pattern:** estimate length by content — **heading count AND body lines**, both required: ≥300 body lines *and* ≥4 headings. If long, assert a content-outline table (in-document `[[#...]]` links) precedes the first body section. If short, assert no such TOC table is present. A specialized content table (neither dispatch masthead nor in-document-heading TOC) does not count either way.
