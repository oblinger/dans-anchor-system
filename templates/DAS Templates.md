...
:>> [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[DAS]] → [DAS Templates](hook://p/DAS%20Templates)
# DAS Templates
...
- **Folder-form facets → one template file per member file, never a template folder** — OpenSpec has no folder-valued template: a schema's `templates/` holds flat files, one per artifact, and a folder shape materializes through each artifact's `generates:` path (their own `specs/<n>/spec.md` works this way). So a folder-form facet ships several flat templates — the primary keeps the facet's name, parts carry a `-part` suffix (Log: [[templates/log.md|log.md]] dispatch page + [[templates/log-entry.md|log-entry.md]] + [[templates/log-single.md|log-single.md]]) — and nothing nests under `templates/` (Constraint C5). Each part binds to its own artifact entry under the facet's schema declaration, so the C2 template↔facet correspondence becomes 1:N with a shared facet spec. *(Verified against Fission-AI/OpenSpec 2026-07-12; see [[SKA OpenSpec Compatibility]].)*
...
Authored: [[templates/backlog.md|backlog]] · [[templates/completed-roadmap.md|completed-roadmap]] · [[templates/decisions.md|decisions]] · [[templates/log.md|log]] · [[templates/log-entry.md|log-entry]] · [[templates/log-single.md|log-single]] · [[templates/messages.md|messages]] · [[templates/prd.md|prd]] · [[templates/query.md|query]] · [[templates/roadmap.md|roadmap]] · [[templates/status.md|status]] · [[templates/testing.md|testing]] · [[templates/track.md|track]]. Tracking-group templates completed 2026-07-12 per [[DAS Tracking Design]]; the remaining per-facet templates are generated by the Sonnet fan-out (one per facet in `facets/`, from its `DAS <X>` spec) once this pattern is ratified.
