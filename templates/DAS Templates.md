...
Seed exemplars authored (F229 item 3): [[status]] · [[DAS Backlog]] · [[prd]]. The remaining per-facet templates are generated by the Sonnet fan-out (one per facet in `facets/`, from its `DAS <X>` spec) once this pattern is ratified.
