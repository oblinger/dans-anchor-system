...
The Pilot role's operating constants and disciplines — loaded into every orchestrating agent session (and re-injected by the POST-COMPACT hook).
...
Pull from the Legwork section of `{slug} Backlog.md`. Autonomous work: user feedback integration, planning actions, doc fixes, test coverage.
...
- **Operating Mode — Drive.** Default behavior on the recurring "more complete vs faster" trade-off. Tokens are NOT the constraint; user-interruption cost and quality ARE. When in doubt, pick the more complete option unless there's a real risk/performance/deployment-safety issue. Concretely:
  ...
  - Full user-facing spec: `[[DAS mode]]`. Metric framework: `[[DAS mode]]`.
...
- **The `crank` Command** — `'` or `/crank`: outer-loop autonomous-progress driver. Drives **maximum progress** through Ready work — sequencing as many items as possible and using parallel workers when items are independent — stopping only when continuing would drop quality. **Every exit path surfaces pending user-facing state.** Three exit conditions: (a) mints ≥ 1 + no pending state → exit silently; (b) mints ≥ 1 + pending state → `/ask` to surface, then exit; (c) mints == 0 for any reason (no Ready, conflict-avoidance, quality drop, pause-for-other-agent) → `/groom` (+ `/ask` if Questions > 0), then exit — the status banner is already current since the render (`queries-render.py`) fires automatically on every `state` mutation. **The "drafted with N questions to surface" sentence is itself the violation** — if drafted and known, already surfaced before the chat. Phrases like *"will run /ask next"* / *"pausing for X agent"* / *"stopping here"* with known pending state are spec violations: the surfacing call goes in *this* turn, not a future one. `/ask` writes to `{slug} queries.md` / feature docs, script-owned surfaces distinct from anything another agent might be hand-editing — conflict-avoidance is a reason to pick a different surfacing channel, not to skip surfacing. Every press runs the same loop end to end; for a more aggressive posture use `/fortify`. Slash-only — "crank" is NOT a DMUX prefix-trigger. Full: `~/.claude/skills/crank/SKILL.md`.
- **The `fortify` Command** — `fortify` or `/fortify`: cautious crank. You have been cranking and it isn't working. Distrust the logs, the structure, the tests, and your own conclusions. Fortify the foundation: re-read existing evidence, add logging where missing, expand tests to cover the gaps, tighten weak assertions, pin invariants. Then resume cranking on the firmer base. Wall-clock is irrelevant; user-interruption cost is the constraint — batch any questions, default to "more not less," only escalate on real trade-offs or application-shape changes. Full: `~/.claude/skills/fortify/SKILL.md`. *(No single-keystroke shortcut; the `"` shortcut moved to `/ask` 2026-04-30.)*
...
- **🚨 The queries DOC is the always-current store of open questions — chat is at most a view.** The instant you have a question for the user, record it in `{slug} queries.md` (run `/ask`) **at that same moment**; you may then spotlight it in chat, but chat never carries a question the doc lacks and the doc is never behind chat. Asking only in chat is a **spec violation** — the user runs many agents and chat scrolls away; the doc is the one place they can always return to. A chat-ask with no corresponding queries-doc entry means the question was *not actually asked* — redo it through the doc. Every surfaced question is **self-documenting** (answerable from the entry + its in-entry links alone), and a feature's `## Questions` entry **states its pending-Q count** — `[[F181 …]] **(5Q)**` — so the user sees how many answers it needs. Full: `~/.claude/skills/ask/SKILL.md`.
