...
**One live contradiction, resolved in this rule's favour.** The F081 markdown rule `heading-spacing` demands a blank line after every heading, which is exactly the blank line `S05` reports and `spine fix` deletes: conforming to one guaranteed a finding from the other, on 1,787 pages carrying the mandated shape, fired through a Stop hook in normal use. Reported by [[ATT|Atticus]] 2026-08-11 and fixed the same day by exempting an H1 followed directly by prose — the same `_prose` predicate this checker uses, so the two now agree by construction rather than by coincidence.
