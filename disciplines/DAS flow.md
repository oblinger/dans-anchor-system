...
name: flow
description: >
  Discipline — the second DAG over anchors. `sources:` in `.anchor` names the
  anchors that supply this one; out-edges are computed by inversion. A flow
...
**Flow is how work becomes visible somewhere other than where it is owned.** An anchor holds its own planning surfaces; flow is the declaration that another anchor draws from them, and the machinery that carries the top of one roster into the next.

**DRAFT, 2026-08-06.** Commissioned in [[TINK312 - Flow: a second DAG over anchors, and the facets that travel it|TINK F312]] and written ahead of its four open questions, which is safe for everything below except the naming of the second member facet — that page is deliberately unwritten until F312 Q1 lands, because its filename and every one of its rule IDs depend on the answer.

## The second DAG

[[DAS anchor-dag]] describes the vault's **containment** graph: every anchor has a place, a breadcrumb up, dispatch links down. Flow is a **different graph over the same nodes** — supply rather than containment — and the two must never be merged.

The proof they are different is the case that motivated flow at all: [[Vector]] sits at the top of the rocks flow and [[LUMEN|Lumen]] at the top of the item flow, and **neither is the parent of the anchors that feed them.** [[MED]] lives under `Topic/`. Reusing `parents:` for flow would either relocate MED or make its breadcrumb lie.

`anchor-dag` names itself a *global, corpus-level* discipline — a property of all the anchors together, verified by walking the whole graph rather than by authoring any one artifact — and explicitly opens a shelf for siblings of that shape. **Flow is the first genuine sibling**: same nodes, different edges, its own invariants (acyclic, every named source resolves, every item owned).

## Declaring an edge — `sources:` in `.anchor`
...
- On [[Vector]]'s `.anchor` — `sources: MED, CMX, NJ, SV, AIS`
...
The key is `sources:` rather than `flows:` because a `.anchor` key is read in isolation: `flows: MED, CMX` on Vector reads naturally as *Vector flows to these*, which is exactly backwards, and it is backwards worst at the root. `parents:` avoids the problem because the noun carries the direction; `sources:` does the same. **The concept stays `flow`** — this discipline, its facets, its graph — with only the key named for precision. *(Pending F312 Q2.)*

## What a flow facet looks like

Every flow facet materializes the same way. This is the whole of what the discipline owns:
...
An item that sits in the export group of anchor `A`'s roster appears in the roster of every anchor whose `.anchor` names `A` as a source.

**An item is written where it is owned, and read everywhere else.** The roster in the item's own anchor is the write surface — edit a line there and the next pass stores it back to `line::`. Every roster downstream of a `sources:` edge is a **rendered copy**, machine-owned in the sense [[DAS Dispatch Table]]'s electric zones already establish: hand-edits are discarded on the next pass, and the zone says so.

This is not a restriction chosen for safety; it falls out of the structure. Each item has exactly one owning anchor, so exactly one roster can hold an authoritative edit. The alternative — reconciling edits made to the same line in two anchors — is the merge problem flow exists to remove, reintroduced one layer down.
...
- **[[DAS stream]]** — the other multi-facet mechanism discipline, and the model this one follows: shared machinery here, per-facet specifics declared by the member. A flow facet's items may each carry a stream.
- **[[DAS file-association]]** — owns the general folder-attaches-to-a-parent placement; flow's folder is one shape of it, specialized by having a roster and an export semantics.

# BRIEF

- **What this is** — the discipline owning the flow DAG (`sources:` in `.anchor`) and the shared shape of a flow facet: folder, one file per item, `key::` parameters, `line::`, a roster whose top group exports.
- **Status: DRAFT.** Written 2026-08-06 alongside [[TINK312 - Flow: a second DAG over anchors, and the facets that travel it|TINK F312]], ahead of its four questions. Q2 could rename `sources:`; Q3 could split the roster out of the folder-note; Q4 could change the identity scheme. None of those change the shape above, which is why it was safe to write.
...
- **No ruleset yet.** `R-flow` is M1 of F312 and waits on Q2 and Q3, since half its rules would assert the key name and the file count.
