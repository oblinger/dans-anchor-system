...
**The block hosts three answer shapes, one lifecycle** (F305, 2026-08-13). The canonical heading is `## Open Items` (`## Open Questions` is the legacy spelling, accepted on read forever; the writer renames on touch). Beside `Q<n>` (choose: labeled options + Recommendation) live `V<n>` (observe: a yes/no verification — no options, no stated lean, its Recommendation necessarily None; minted with `--why-user` naming the human faculty, F240) and `U<n>` (perform: a user action; minted with `--why-user-action`, F259). Addressing is uniform — `state <define|set|resolve|remove|show> <anchor> <doc> <Q|V|U><n>` (or `Q+`/`V+`/`U+` to mint) — with per-letter monotonic numbering and block-IDs `^F<n>-V<n>`. Resolve vocabulary follows the shape: a Q takes an option letter or `none`; a V takes `yes`/`no`/`none` — **answered no records the outcome and closes nothing**, the feature is simply not verified; a U takes `done`/`none`. Everything below about zones, stamping, and migration is kind-blind: the block is deleted only when empty of **all three kinds**.
