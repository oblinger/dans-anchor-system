...
**Rollout.** Live now (2026-07-18): the field, and `state Q define`'s **mechanical auto-resolve** of `waste`/`priority` — *no Warden rule needed*, since the `state` mint is the single funnel every agent's question passes through (Warden's separate job is only the *stop*-gate: don't stop with an unsurfaced question, [[SKA Backlog#^F267|F267]]). The Damage line is **soft-required**: `state` warns when it is missing and surfaces by default, so no caller breaks; it flips to hard-required once the `/ask` + `/feature` mint templates carry the line. A mis-typed category is refused outright (a typo must not pass as askable). See [[SKA Backlog#^F270|F270]].
