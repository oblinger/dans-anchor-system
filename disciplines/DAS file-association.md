...
description: >
  ...
  [[DAS dated-entry-stream]] (Discussion, Log); non-dated typed associations (Brief,
...
- **Dated?** — does each item carry a date and sort reverse-chronologically? **Yes** → it's a [[DAS dated-entry-stream]] (Discussion, Log) — the dated specialization adds newest-first ordering, prepend semantics, and ISO-date entry-file naming. **No** → a static typed association (Brief, Decisions) — it needs nothing beyond this umbrella.
...
- **Per-item file naming** is dimension-specific: dated streams prefix each file with an ISO date (`YYYY-MM-DD — <Title>.md`) — see [[DAS dated-entry-stream]]; non-dated collections name by title alone.
...
|---|---|
| [[DAS dated-entry-stream]] | The **dated** case — every item dated; newest-first ordering; prepend (not append) semantics; ISO-date entry-file naming; a parallel per-facet entry skeleton. Examples: Discussion, Log. |

**Non-dated typed associations** (Brief, Decisions, …) cite *this umbrella directly* — they need nothing the umbrella doesn't already provide, so they earn no specialization (per [[DAS granularity]]: specialize only when extra rules warrant it; dated does, static-single doesn't).
...
> Discussion is a [[DAS dated-entry-stream]] (dated). Methods supported: 1 (inline, default) and 2 (sibling file).
> Brief is a static typed association ([[DAS file-association]], non-dated). Methods: 1 (inline `# BRIEF`, default) and 2 (sidecar); 3 if a parent accumulates many.
