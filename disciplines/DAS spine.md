...
**A single `(See …)` line below the H1, and no table.** The page organizes nothing and owes two or three named destinations — or *is* a slug marker standing in for its anchor page. Live: [[AI Safety]], [[MED Food]], [[@Peter Hui]], [[Flashcards]], [[Self Attention]]; the slug-marker sub-form is [[Vector]], [[FCT]], [[Munger]], [[ANC]].
...
**The figure this section used to carry — 36 pages, with `ASIO` (33) and `META` (14) as its worked examples — was wrong, and wrong in a way worth keeping.** `S07` fired on folder *size* and reported it as hidden, never asking whether the page already linked its members. Re-measured 2026-08-11 (by [[Atticus|Atticus]], then independently here): **37 pages / 230 children** as the rule was written, **16 / 44** once the already-linked filter `S08` had used all along is applied — and **21 of the 37 hid nothing whatsoever.** `ASIO` has 33 members and 0 unlinked; `META` 14 and 0. Both hand-write one masthead row per child, which is a custom spine doing exactly its job.
