...
**Agent Drive — the handoff promise.** Drive's posture points two ways at once. Toward the work: *stopping is the costly action, not continuing.* Toward the user: **every handoff back hands them the biggest chunk of progress it can — the deck cleared of everything the agent could do itself — and the most decisions it can — every pending question recorded in state, none left in chat to scroll away.** So the user is never stranded: at any stop they can either answer a pending question or press `'` to crank on a groomed frontier, always. This is the *Agent Drive* idea — agent and user both making maximum progress, no needless round-trips — and it is enforced mechanically by the F244 stop gate: a work-armed stop is illegal while the anchor's grooming worklist is non-empty (see [[DAS State]] `state groom-list` and [[SKA workflow]] § Never strand the user). The handoff *closes* by telling the user where they stand: a work-armed turn's last line echoes the status banner (`Ready N · Questions M · Verify K`, from `state groom-list`), so at a glance they can answer a batch of questions or crank.

**The same promise covers verification, and it binds harder there.** A pending question at least *needs* the user; a Verify row often does not, and one that reaches them anyway is the verification-side form of the chat-only question — it takes a slot on the handoff without earning it. So a handoff runs every check it can answer itself first. The test is [[DAS verification]]'s single question — *would the user's answer be any better than the agent's?* — and when the answer lives in a file, a log, or a probe, it is the agent's to run in the turn that does the work, not the user's to be asked about. What survives is what needs a human faculty: taste, preference, ratification, or an observation the user makes in ordinary use. That is what `--why-user` forces the agent to name at the mint ([[F240 — Verification ownership gate — who-is-better-positioned enforced at Verify mint|F240]]) and what audit-q **C47** flags after the fact on a row phrased as a machine event. So the banner's `Verify K` reads as *K things only you can judge*; the moment it means *K things nobody got around to running*, the handoff has stopped being one. A stop gate that refuses to close while an agent-verifiable check sits unrun is the obvious next tightening and is deliberately **not** part of this — the discipline is stated here, and enforcement stays where F240 already put it, at the mint.
