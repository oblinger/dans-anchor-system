...
- **`F<n>` — a feature.** Backed by a feature doc under `{slug} Design/{slug} Features/`; the backlog row links to it. `F` numbers are monotonic, never reused.
...
- **`M-<Name>…` — a roadmap entry.** A *nested* item in `{slug} Roadmap.md` (per [[DAS Roadmap]]); its handle is `M-<Name>.<path>`, where the name-path already encodes its position in the tree. Entries nest; the roadmap is a hierarchy.
...
**Decisions live on the feature's record; rows carry work (ruled 2026-07-14).** A question that (re)decides or extends an existing feature's design belongs on that feature doc's `## Open Questions` — the next Q-number in *that feature's* namespace, reopening the doc (ask-format Phase 3) even when the feature is `[Done]`; the backlog row rebrackets `[Questions]` to match. Minting a spin-off `T` row to *host a decision* is forbidden: a row whose entire body is decision context and whose only output is an answer is a feature-doc question in a work-item costume — once `[Done]` it scrolls into the backlog graveyard and the decision record scatters. Inline `T`-row Qs are for forks in the row's *own standalone work* (strategy, timing, approach of the task itself). Corollary — **supersede back-edges are mandatory**: whenever a new answer supersedes an earlier resolved Q, wherever the two live, the old resolution entry gets a one-line *"superseded {date} → see X"* in the same pass. (Ruled after HA `T001`: a re-decision of F113's canonicalize question was spun into a T-row; its answer ended up recorded only inside a *third* row's `Next:` line while both the question's own record and the parent feature went stale.)

## The groom frontier (activity 2)

**The frontier is the set of tasks that could be next for execution** (defined 2026-07-05, [[F228 — Groom frontier — groom plans the frontier to Ready; query asks about it|F228]]): rows in the backlog's **`## Now` or `## Next` horizons** (plus `## Active` / `## Ready`, which are already past it), **feature docs linked from a Now/Next row**, and items **soon on the relevant roadmaps** — the next unmet milestone of `{slug} Roadmap.md` when the anchor has one, *a milestone that could plausibly be executed soon if fully specified*. `## Later` and the icebox are *not* frontier; they are touched only on explicit invocation (`/groom later`, `/groom icebox`).
...
Grooming a frontier item means **planning it out until you know, as concretely as possible, how you would execute it** — then recording that knowledge as one of **five explicit groomed states**, never leaving the row in an unknown state. Each state is honest only if its body carries a specific contract a checked `R-backlog` rule enforces; the canonical per-state table (state → body contract → enforcing rule) lives at [[R-backlog]] § The groomed states — Executable (`[Ready]`/`[Active]`, R-backlog-02), Questions (`[Questions]`, R-backlog-05 + the R-query bar), Blocked / Waiting (`[Blocked …]` / `[Waiting …]`, R-backlog-06/07), Verify (`[Verify]`, R-backlog-04), Watching (`[Watching …]`, R-backlog-04/07).
...
**Tier 1 (agent-immediate).** After `/groom` (or `/ask`), the agent's own chat turn contains **no question directed at the user** (no "Want me to…?", "Should I…?", "which do you prefer?"). Every user-answerable item is present in `{slug} queries.md`, self-documenting, with a count. `audit-q` is clean before the doc is surfaced.
...
| **Render** | mechanically paint `Q.md` + `{slug} queries.md` from current state, then glance | `queries-render.py` (fired automatically via `state`) |
