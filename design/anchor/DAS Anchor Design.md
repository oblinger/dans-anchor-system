...
:>> [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[DAS]] → [DAS Anchor Design](hook://p/DAS%20Anchor%20Design)
...
| **Skills**                                 |                                                                                                                  |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| [[DAS Create\|/create]]                    | Create a new thing — anchor, feature, work product, spec, rule.                                                  |
| [[DAS Migrate\|/migrate]]                  | Change anything about an anchor — slug, traits, location, structure, naming.                                     |
...
| [[DAS WP\|/wp]]                            | Mint a dated work-product folder — polished human+agent outputs.                                                 |
...
| [[DAS Anchor Toolkit\|/anchor]]            | The toolkit — scan, config, status, docs-vs-source audit scripts.                                                |
| [[DAS Install\|/install]]                  | One-time per-machine install of the anchor CLI tools.                                                            |
|                                            |                                                                                                                  |
| **Facets**                                 |                                                                                                                  |
...
|                                            |                                                                                                                  |
| **Library**                                |                                                                                                                  |
| **`ha`**                                   | The HookAnchor CLI — resolves slugs and `[[names]]` to paths (`ha -p`), dumps the anchor index, desc-syncs.      |
...
Anchor's contract: **everything lives in an anchor, and an anchor is always navigable.** A named folder becomes an anchor by carrying a `.anchor` marker (slug + description + traits); its `{NAME}.md` page opens with the dispatch masthead that routes one or two clicks to anything inside (per [[DAS progressive-disclosure]]); `ha` resolves slugs and basenames to paths so wiki-links stay live vault-wide. The lifecycle runs left to right: `/create` mints the shape → `/migrate` and `/move` evolve it in place or relocate it (updating every path-dependent system) → `/publish` optionally ships its public page → `/yore` retires content as date-prefixed archives. `/wp` mints dated work-product folders inside any anchor along the way.

Boundaries: **the substrate serves the other subsystems** — Tracking, Design, Code, and the rest are traits and folder shapes declared *on* an anchor, never parallel structures beside it. **Hygiene audits what Anchor defines** — the R-anchor family is authored here, fired by Warden and `/audit`. **Code anchors extend the marker** — the `code:` and `mirror:` keys (Sparse-Linked, Two-Way Doc Mirror per [[SKA Decisions]] D12) are `.anchor` vocabulary owned by this group, exercised by the Code subsystem.
