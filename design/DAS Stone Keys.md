...
`tempo::` is written by the step from the workflow's `raise::`; `importance::` may be rewritten by a step's annotation. The Traffic spine's `raise` field has no key — it is the current step's evaluated `when`, recomputed each cycle so it cannot rot ([[Tink635 - Loop mechanism: a stone that carries a workflow, so Traffic can run it|TINK F635]] § Resolved).
