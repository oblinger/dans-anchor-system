---
description: "product requirements for the Lumen morning routine"
---
# DAS Lumen PRD
Why the morning routine exists, who it serves, and what would make it fail.

| -[[DAS Lumen PRD]]- | → [[kmr]] → [[SYS]] → [[Bespoke]] → [[SKA]] → [[DAS]] → [design](hook://design) → [DAS Lumen PRD](hook://p/DAS%20Lumen%20PRD)<br>: product requirements for the Lumen morning routine |
| --- | --- |
| Related | [[DAS Lumen Design\|Design]],  [[DAS Lumen]] (user docs),  [[skills/lumen/SKILL.md\|SKILL]], |
...
Two failure modes follow, and they are the ones Lumen exists to prevent:
...
- **Domain capture.** Whichever agent the user opens first sets the day's agenda. Work has the most surface area and the loudest backlog, so work wins by default. Health, which the user has written into every life-framing they have ever authored ([[Luna Domains]]), loses by default.

## Who it serves

One user, once a day, in five to ten minutes, at the start of the day. That constraint is load-bearing and shapes everything below: **attention is the scarce resource, not agent time.** Lumen may read for a minute if it means the user reads for ten seconds.
...
- **Must not duplicate domain-agent tracking.** Work items live in their own anchors and reach Lumen through `Q.md` federation, not by being copied into Luna's backlog.
...
**Counter-signal to watch for:** if the user routinely acts on something *other* than what Lumen surfaced, the selection rule is wrong, not the user. Prioritization is provisional until [[F001 — Luna onboarding and charter capture|Luna F001]]-Q4 settles it, and this counter-signal is the evidence that should settle it.
...
- Live watch → session delivery. That is [[F003 — Live watch-to-session channel — speak to Luna from the wrist|Luna F003]]; the once-a-day sweep is the reliable floor it builds on.
- Automatic addressing detection in [[MUSE]]. Until it exists, Lumen reads all non-suppressed items past the watermark — slightly noisier, zero new dependencies, and shippable today.
